...
| 데이터 의미 | 형태·필수성 | 값 없음·조건 |
| --- | --- | --- |
| 연결된 확정 세계관 대상 | 식별자·조건부 | 새 대상 추가 제안이면 `null` |
...
- 후보 수정:
  - 분류, 대상명, 설정명, 최종 반영값, 최종 반영 방식 중 사용자가 변경한 값
  - 원문 근거·LLM 원본·비교 기준값은 FE가 재조립하지 않는다.
- 후보 확정:
  - 후보 식별자, 최종 반영 방식, 최종값, 선택적 검토 메모
- 후보 제외:
  - 후보 식별자, 사용자의 제외 의도, 선택적 검토 메모
- 비교 재시도:
  - 후보 식별자와 사용자가 수정한 분류·대상·설정명

### 6. BE와 협의할 범위·상태값

- 위 데이터 의미를 어떤 API·DTO로 결합하거나 분리할지는 Swagger 설계 시 확정한다.
- 캐릭터 후보와 세계관 후보의 배치 집계를 한 응답으로 제공할지 FE가 두 조회를 합산할지 확정한다.
- 동일 설정명의 현재값 비교와 재비교 전환을 원자적으로 처리하는 Backend 계약을 확정한다.
...
- 버전 충돌: 사용자의 입력을 유지한 채 최신값 다시 불러오기 안내
- 저장 성공: 목록 개수·상세·선택 대상·버전을 최신 상태로 갱신하고 성공 안내
- 편집 중 다른 대상 선택·모달 닫기: 작성 내용을 취소할지 확인
...
| 설정별 현재값과 일치하는 최신 후보 근거 | 조건부 | 직접 입력·수정이거나 근거가 없으면 `null` |
| 후보 확정 이력 | 설정별 목록·필수 | 이력이 없으면 빈 목록 |
...
- 같은 `분류 + 대상명`이 이미 있으면 생성·수정을 완료하지 않는다.
- 같은 대상 안에 같은 설정명이 이미 있으면 추가·이름 변경을 완료하지 않는다.
- 설정명과 설정값은 화면에 표시하는 문자열을 전달하고 JSON 전체를 FE가 다시 조립해 덮어쓰지 않는다.

### 6. BE와 협의할 범위·상태값

- 목록 검색의 대상명·설정명·설정값 부분 일치와 고정 정렬을 제공하는 데이터 계약
- 현재값과 일치하는 최신 확정 후보 근거 및 후보 이력 제공 방식
- 직접 입력·수정한 현재 설정을 후보 근거 없음으로 구분하는 방식
- 대상·설정 추가와 수정의 낙관적 버전 충돌 응답 계약
- 구체적인 API 경로·HTTP 메서드·DTO 이름은 Swagger 설계 시 확정
...
- 구체적인 API 경로·HTTP 메서드·전체 DTO 확정
