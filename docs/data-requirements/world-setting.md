...
- 도감: 현재 대상과 같은 분류만 이름·별칭 검색. 18개 단위 서버 페이지네이션, 데스크톱 3열/작은 화면 2열. 기본 이미지 선택과 저장/취소, 하단 저장 영역은 스크롤해도 접근 가능하다.
...
- 도감 로딩/검색 결과 없음/오류/재시도를 구분한다. 저장 오류·충돌은 선택을 지우지 않는다. 파일 조회 실패는 번들 분류 기본 이미지로 대체한다.
