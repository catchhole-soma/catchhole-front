...
- `GET /api/v1/world-image-catalog?category=&q=&page=&size=`는 `data.content`를 사용한다. `PATCH /api/v1/works/{workId}/world-settings/{id}/image`에 `{catalogId,version}`을 보내며 null 선택은 기본 이미지 복귀다. 생성 SDK/TanStack options를 사용한다.
