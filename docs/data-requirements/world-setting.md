...
- 오른쪽 상세는 후보별 설정명을 key row로 표시하며 각 row의 후보 ID와 비교·검토 상태는 유지한다.
...
- 같은 분류·대상·설정명으로 여러 값이 추출되면 후보를 여러 row로 반복하지 않는다. AI가 보완값을 최종 제안값 하나로 합치고 각 1차 후보의 원문 quote·offset은 한 row 아래에 모두 보존한다.
...
- 단일 확정은 설정 하나만, 대상 그룹 확정은 사용자가 선택한 여러 설정만 원자적으로 추가·수정하며 JSON 전체를 교체하지 않는다.
- 대상 그룹 확정에서 여러 설정을 바꾸면 한 트랜잭션을 하나의 확정본 revision으로 보고 버전을 한 번 증가시키며, 함께 확정된 후보는 같은 최종 적용 버전을 사용한다.
- 같은 그룹 확정 안에서 먼저 적용한 `ADD`로 대상이나 key가 생긴 것은 내부 변경으로 취급해 나머지 선택 후보를 재비교하지 않는다.
...
> - 같은 세계관 대상 그룹 안의 선택 key 일괄 확정·일괄 제외만 지원한다. 서로 다른 대상 그룹을 한 번에 처리하는 기능, 근거 직접 수정, 설정집 추출 후보는 지원하지 않는다.
...
- `CONFLICT`는 `내용 확인 필요`로 표시하고 서로 다른 추출값과 각 1차 원문을 모두 보여준다. 기본 선택하지 않으며, 사용자가 `수정`에서 하나의 최종 설정값을 저장하기 전에는 반영 확정을 잠근다. `선택 항목 제외`는 그대로 허용한다.
...
- 설정 row별 선택 체크박스·비교 상태·제안된 반영 방식·출처 회차
...
- 설정명(key)을 row 제목으로, 설정값(value)을 기존 확정값 → 최종 제안값 diff로 표시
- 이미 검토된 후보의 최종 작업·최종 분류·대상·설정명·설정값
- 그룹 하단의 선택 개수, `선택 항목 제외`, 선택 결과에 따른 `N개 설정 반영`·`제안대로 N개 제외`·혼합 확정 문구
...
- 설정 체크박스로 이번 그룹 액션 대상을 선택
- `선택 항목 제외` → 선택 후보를 한 요청으로 `DISMISSED` 처리
- `N개 설정 반영` 또는 제외·혼합 확정 → 선택 후보를 대상 그룹 단위 단일 트랜잭션으로 확정
- 확정 대상이 없으면 Backend가 대상을 한 번만 생성하고 선택 key를 함께 반영
- 설정 row의 `수정` → 해당 후보의 반영안 수정
- `EXCLUDE` 제안도 그룹 확정 요청의 최종 작업으로 전송해 `DISMISSED` 처리한다. 모든 선택이 `EXCLUDE`이면 `제안대로 N개 제외`, 반영 작업과 섞이면 반영·제외 개수를 함께 표시하며 확정 버튼을 잠그지 않는다.
...
- 비교 실패: 실패 row와 이유를 표시하고 그룹 확정을 잠근 채 `재비교 다시 시도`와 `선택 제외` 제공
...
- 그룹 확정 성공: 목록·집계·상세를 최신 상태로 갱신한다. 신규 대상을 일부 key로만 생성했다면 남은 후보를 자동 재비교하고 같은 그룹을 유지한다.
...
- 대상 그룹 확정:
  - `batchId`, 같은 그룹의 선택 후보별 식별자·최종 분류·대상명·설정명·반영 방식·설정값·선택적 검토 메모
...
- 대상 그룹 제외:
  - `batchId`, 같은 그룹의 선택 후보 식별자 목록, 사용자의 제외 의도, 선택적 검토 메모
- 비교 재시도:
...
- Backend는 같은 그룹의 선택 후보를 한 트랜잭션에서 검증·반영한다. 신규 대상은 한 번만 생성하고 그룹 내부에서 먼저 적용한 key 변경은 다음 선택 후보의 충돌로 판단하지 않는다.
- 외부 변경으로 비교 문맥이 달라졌으면 선택 후보를 하나도 반영하지 않고 영향 후보를 `RECOMPARISON_REQUIRED`로 저장한 뒤 409와 `ROW|GROUP` 영향 범위·사유·후보 ID를 반환한다.
