...
- 오른쪽 상세 header는 `분류 · 대상명`과 설정 항목 개수를 표시하고, 같은 분류·이름으로 조회된 후보를 설정 항목별 Git diff row로 묶는다. 이 묶음 자체가 같은 주체라는 판정을 뜻하지는 않는다.
...
- 분석 대상 회차 범위와 회차 수
- 캐릭터·세계관을 합친 전체 후보 수와 반영됨·제외됨·직접 확인 세 가지 요약
- 비교 대기·진행 중이거나 해당 회차의 자동 반영을 기다리는 후보는 별도 `분석 중` 상태로 표시
- 각 후보 종류 탭에는 직접 확인·분석 중 수를 표시하고 설명 없는 완료/전체 분수를 쓰지 않음

**왼쪽 필터·목록**

- 검토 상태: 전체·미처리·반영됨·제외됨을 캐릭터 검토와 같은 버튼 선택 그룹으로 표시
...
- `SCOPE_MISMATCH`는 `범위 비교 필요`로 표시한다. `비교한 기존 설정`에는 비교된 기존 범위·항목명과 당시 값을, `원문에서 추출한 설정`에는 원본 범위·항목명과 추출값을 나란히 표시한다. 모바일에서는 같은 순서로 세로 배치한다. 후보 제목과 최초 편집값에도 원문 경로를 사용하며 AI 제안 경로로 대체하지 않는다. 기존의 `SCOPE_UNRESOLVED` 범위 미정 안내는 유지한다.
- `SUBJECT_UNRESOLVED`는 `대상 연결 확인 필요`, `SCOPE_UNRESOLVED`는 `범위 확인 필요`, `BATCH_LIMIT_EXCEEDED`는 `비교 분량 확인 필요`, 사유가 없는 보류는 `검토 필요`로 표시한다. 대상 연결이 보류된 후보의 비교값 영역은 `비교 대상 미정`으로 표시하고 기존값 부재로 단정하지 않는다. 보류 안내 제목은 `검토 안내`, 실패 안내는 `비교 실패 안내`이며, 실제 비교 완료 사유에만 `AI 비교 판단` 제목을 사용한다.
- `SCOPE_UNRESOLVED`에도 양쪽 경로와 값을 표시한다. 범위 없는 원문은 `범위 미정 › 원문 설정명`으로 표시하고, AI가 다른 기존 설정명이나 제안값을 반환해도 원문 제목·값·초기 편집값을 덮지 않는다. 반영할 범위와 설정명이 다르면 사용자가 수정창에서 명시적으로 정하고 저장한 뒤에만 그룹 확정을 허용한다.
- 선택적 `comparisonDiagnostics`가 있으면 마지막 비교 시도의 `selectedProperties`에서 범위·설정명만 `마지막 비교에서 선택한 기존 설정`으로 표시한다. 확정된 연결이 아니라는 안내를 동반하며 `rule`, `candidateRefs`, 대상 ID·임시 대상 키는 사용자 화면에 노출하지 않는다. 과거 응답처럼 진단이 없으면 해당 영역을 생략한다. 비교 실패의 원시 오류나 오래된 성공 사유를 AI 판단으로 표시하지 않는다.
- `manualReviewAvailable` 직접 확인 안내는 밝은 바탕·주황색 강조선으로, 저장한 반영안 안내는 연한 파란색 배경으로 구분한다. 범위 비교 안내와 일반 확인 안내를 중복해 쌓지 않는다. 제목과 본문 명암비는 모두 4.5:1 이상이며 저장 완료도 작품 설정 반영 전 단계임을 문장으로 알린다.
- 그룹 기본 설명은 `같은 분류·이름으로 묶인 후보를 항목별로 검토합니다.`로 표시한다. 미수정·비교 완료·검토 대기 후보들이 서로 다른 실제 `targetWorldSettingId`를 가질 때만 서로 다른 기존 연결을 확인하도록 안내한다. 실제 대상 ID가 null인 후보들의 동일성이나 보류 원인을 비교 묶음 ID·진단 기록으로 추정하지 않는다. 자동 반영 시 기록된 `automaticReviewHoldReason`가 있을 때만 같은 대상 확인 등 사유별 배지를 표시하며 null인 과거 후보는 일반 직접 검토 안내를 유지한다. 이 안내만으로 확정 조건을 바꾸지 않는다.
...
- 범위가 다른 검토 필요 후보는 편집을 열어 원문과 기존 범위를 비교한 뒤 최종 범위·항목명·반영 방식을 선택한다. 단순히 편집창을 열거나 AI 비교가 완료된 것만으로 `모두 확정`을 허용하지 않으며, 서버에 유효한 반영안을 저장한 뒤에만 확정할 수 있다. 다른 보류 후보가 남았으면 그룹 확정은 계속 잠근다.
...
- 후보 수정:
  ...
  - `REVIEW_REQUIRED + BATCH_LIMIT_EXCEEDED` 후보는 `출력 한도 검토`로 표시하고 그룹 확정을 잠근다. FE는 수정 모달에 후보의 1차 추출 범위·설정명·값과 기본 `ADD`를 미리 채우되 자동 저장하지 않는다. 작가가 반영 방식과 최종값을 확인해 수정안을 저장한 뒤에만 확정을 허용하며, `CONFLICT`는 원문별 추출값을 표시하고 하나의 최종값을 직접 확인해야 `conflictResolved=true`로 전환한다.
    - `CONFLICT`의 미리 채운 추출값을 그대로 저장하거나 공백·줄바꿈만 바꿔 반영하려 하면 오류를 표시하고 입력과 그룹 확정 잠금을 유지한다. 후보 결정 PATCH는 보내지 않으며, 반영하지 않는 `EXCLUDE`는 허용한다.
...
> **분류 이미지 카드 (GH-194)**: 8개 분류 모두 이미지 상단·제목과 설명 하단의 카드로 표시한다. `전체 보기`는 사용자가 선택한 A안(넓은 세계를 내려다보는 풍경)을 사용한다. 그림은 공통 분류 예시이며 작품의 실제 설정 이미지가 아니다. 3:2 비율의 로컬 WebP를 사용하고, 모바일에서도 같은 비율로 두 열을 유지한다. 제목과 설명은 이미지에 겹치지 않는 실제 텍스트로 제공한다. 처음 네 이미지는 즉시, 나머지는 지연 로딩하며 이미지 영역의 높이를 미리 확보한다. 키보드 포커스와 모션 감소를 지원한다. 생성 프롬프트·이미지 매핑은 [`src/assets/world-categories/README.md`](../../src/assets/world-categories/README.md)에 기록한다.
...
![이미지 분류 카드 데스크톱](../screens/gh194/world-categories-desktop.png)

[분류 모바일 390px](../screens/gh194/world-categories-mobile.png) · [분류 320px](../screens/gh194/world-categories-320.png)

[대상 목록 데스크톱](../screens/gh194/world-subject-list-desktop.png) · [상세 모달 데스크톱](../screens/gh194/world-detail-desktop.png) · [목록 모바일](../screens/gh194/world-subject-list-mobile.png) · [320px 상세 편집](../screens/gh194/world-detail-mobile.png)

**Pencil 시안**

아래 분류 선택 시안은 이미지 적용 전 상태다. GH-194의 최신 분류 카드 기준은 위 구현 캡처이며, Pencil MCP 연결 오류로 `RSX1d`·`JezJo` 프레임 동기화가 남아 있다. 목록·상세·편집 시안도 아래 새 목록·모달 계약으로 대체됐으며 동기화가 필요하다.
...
> **디자인 원본**: `design/catchhole.pen`의 `Active / Dashboard / World / ...` 프레임은 동기화 대기 상태다. GH-194는 사용자 승인 내용과 구현 캡처를 최신 기준으로 사용한다.

> **Theme V2 화면 계약**: 분류 선택 → 대상 카드 목록 → 상세 모달 순서로 탐색한다. 목록의 `분류 선택으로` 버튼은 분류·검색·정렬·페이지를 비우고 이미지 분류 선택으로 돌아간다. 목록 안의 분류 버튼·선택 메뉴는 제공하지 않는다. 검색·정렬은 하나의 밝은 surface로 묶고 카드와 상세 모달도 Theme V2 토큰을 사용한다.

> **GH-194 범위**
> - 대상 목록은 데스크톱 세 열, 1100px 이하 두 열, 600px 이하 한 열의 카드로 표시한다.
> - 카드에는 이름과 설정 개수만 기본 표시한다. 전체 조회에서만 분류 배지를 추가하며, 검색 결과에는 API가 반환한 일치 설정을 한 줄로 표시한다. 대표 설정 생성이나 카드별 상세 선조회는 하지 않는다.
> - 모든 화면 크기에서 자동 대상 선택을 하지 않는다. 대상 클릭 또는 `settingId` 공유 링크에서만 상세를 조회하고 모달을 연다.
> - 상세·생성·수정·작성 취소 확인은 Radix Dialog로 구현한다. 포커스 제한·Escape·배경 클릭·스크롤 잠금과 종료 후 목록 초점 복원을 제공하고, 상세 본문만 스크롤하여 닫기 버튼을 유지한다.
> - 목록이 비어 있거나 조회가 실패해도 선택된 대상의 상세·오류 모달은 독립적으로 표시한다. 잘못된 목록 페이지를 보정할 때 선택 대상을 지우지 않는다.
> - 새 대상·설정 추가와 수정은 지원하고 삭제·보관·복원은 후속 범위다. 직접 입력·수정의 API·버전 충돌·근거 계약은 유지한다.
> - `/demo`도 목록을 먼저 표시하고 같은 상세 모달을 사용한다. fixture는 안내문과 체험 완료 액션을 계속 사용할 수 있도록 비차단 dialog를 사용하며 API나 저장소에 연결하지 않는다.

### 표시·정렬 정책

- 목록 한 항목은 `대상명 + 설정 개수`다. `전체`에서만 분류 배지를 표시하고 최근 수정 정보는 상세에서 확인한다.
...
- 실제 목록 여유 높이와 카드 열 수를 측정해 한 페이지 12·18·24개 중 수용 가능한 가장 큰 단위를 표시한다. 작은 화면은 최소 12개로 스크롤을 허용하며, 3열·2열 모두 페이지 단위가 맞는다. 전체 결과의 마지막 페이지는 남은 개수만 표시한다.
- `worldSize`로 페이지 경계를 저장해 새로고침·공유·뒤로가기에 복원한다. 열 수 또는 화면 수용량이 바뀌면 이전 페이지 첫 항목이 포함된 페이지로 보정한다. 검색·정렬 변경은 크기를 유지하고 첫 페이지로 이동하며, 분류 선택·분류 선택 복귀·탭 전환은 크기를 초기화한다.
- 대상 선택 전에는 상세 API를 호출하지 않는다. 크기에 관계없이 목록 선택 후 모달을 표시한다.

### 1. 화면에 표시할 데이터

**상단 탐색**

- 반복되는 세계관 제목·설명은 목록에서 생략하고 뒤로가기·분류명·조회 개수·추가 버튼을 한 줄로 배치한다. 작은 화면에서는 뒤로가기를 접근 가능한 이름과 44px 영역을 가진 화살표로 표시한다.
- 대상명·설정명·설정값 통합 검색
- 현재 분류 이름·조회 대상 수와 `분류 선택으로` 버튼
- 분류·가나다순 / 최근 수정순 정렬
- `새 대상 추가` 버튼
...
- 검색 시 일치한 설정명·설정값 요약
- 페이지네이션
- 카드 최소 높이는 데스크톱 104px, 모바일 94px이며 긴 이름·분류 배지·검색 일치 내용은 필요한 만큼 높이를 확장한다.
...
- 검색어·분류·정렬·페이지 변경
- 대상 선택 → `settingId`를 추가해 상세 모달 표시. 브라우저 Back/Forward와 새로고침으로 선택을 복원
- 모달 닫기 → `settingId`·`modal`만 제거하고 검색·분류·정렬·페이지·목록 스크롤을 유지. 카드로 초점을 돌려주며 오래된 `settingId` 전용 공유 링크는 전체 목록으로 복귀
...
- 세계관 페이지당 대상 수: `worldSize=12|18|24` (API에서는 `size`로 전송, 설정 검색의 `size=20`과 독립)
- 선택 대상: `settingId`
- 새 대상 추가 모달: `modal=world-setting-create`
- 대상 정보 수정 모달: `modal=world-setting-edit&settingId={id}`

설정 인라인 편집 여부와 미저장 입력값은 대상 ID에 묶인 클라이언트 전용 상태로 두고 URL에 저장하지 않는다. 브라우저 Back으로 상세를 닫아도 같은 화면의 Forward에서 해당 대상의 입력을 복원하며 다른 대상에 섞이지 않는다.
...
- 확정 세계관 0건:
  ...
  - `회차 분석하기`, `새 대상 추가` 제공
- 검색 결과 0건: `검색 조건에 맞는 세계관 설정이 없습니다`와 `검색어 지우기` 제공. 검색어를 지워도 현재 분류는 유지한다. 검색어 없이 분류가 비었으면 `이 분류에 등록된 대상이 없습니다` 표시
- 목록 조회 실패: 오류 안내와 다시 시도
- 상세 조회 실패: 목록은 유지하고 모달에서 다시 시도·닫기 제공
...
- 900px 이하: 검색·정렬 컨트롤을 한 열로 쌓고 320×568에서도 상세 본문·마지막 편집 행·저장 액션을 스크롤로 접근 가능하게 한다
...
- 공개 `comparisonReason`는 정제된 자연어로 표시한다. 원문·고유명사·설정값에는 문구 치환을 적용하지 않는다. `comparisonErrorMessage` 원문을 재비교 안내로 표시하지 않고 알려진 상태에 맞는 다음 행동을 안내한다.
- 정상 비교의 `GENERAL_UNCERTAINTY`는 `대상·내용 확인 필요`로 표시한다. 수동으로 해결 가능한 `FAILED` 후보는 저장 상태를 바꾸지 않고 `검토 필요`와 자동 비교 미완료 안내를 제공한다. 수동 저장 후 확정 조건은 유지하며 원문·마지막 비교 경로를 보존한다. 수동 확인 불가 실패·사용량 부족은 기존 재시도/재개 안내를 유지한다. 그룹 검토 수는 정상 검토 요청과 수동 확인 가능한 실패를 중복 없이 합치며 전체 요약 집계는 변경하지 않는다. 반영 방식 필터의 `REVIEW_REQUIRED` 이름은 `AI 판단 보류`로 하여 모든 직접 확인 대상을 뜻하는 것으로 오해하지 않게 한다.
...
개인 이미지 확장: 선택창에서 `공용 도감`과 `내 이미지`를 전환한다. 개인 원본·썸네일·파일명은 브라우저 암호화 후 저장하며 복구키는 탭 메모리에만 유지한다. 개인 선택은 `privateImageId`·`vaultId`·`source: PRIVATE`로 구분하고, 잠긴 상태에서는 기본 그림으로 오인시키지 않도록 `잠긴 내 이미지`를 표시한다. 복구키를 입력하면 복호화한 Blob URL을 표시하고, 상세 캡션은 `내 이미지`로 바뀐다. 크기 제한·보관함·삭제·검증 계약은 [개인 이미지](../private-world-images.md)를 따른다. 아래 도감 오류의 기본 이미지 대체 규칙은 공용 이미지에만 적용한다.

- 대상 카드: 90×72px(모바일 78×64px) 썸네일, 이름, 설정 개수. 기본 카드 높이 104px/모바일 94px와 기존 12·18·24 자동 페이징을 유지한다.
- 상세 헤더: 대표 이미지, `공용 예시 이미지`, `이미지 변경`. 설정을 편집 중일 때 이미지 변경은 잠근다.
- 도감: 현재 대상과 같은 분류에서 `장르 추천`으로 시작하고 `전체 도감`으로 모든 장르 이미지를 선택한다. 두 범위에서 이름·별칭 검색을 유지한다. 18개 단위 서버 페이지네이션, 데스크톱 3열/작은 화면 2열. 기본 이미지 선택과 저장/취소, 하단 저장 영역은 스크롤해도 접근 가능하다.
- `GET /api/v1/world-image-catalogs?category=&q=&page=&size=`는 `data.content`를 사용한다. `PATCH /api/v1/works/{workId}/world-settings/{id}/image`에 `{catalogId,version}`을 보내며 null 선택은 기본 이미지 복귀다. 생성 SDK/TanStack options를 사용한다.
- 목록·상세의 `image`에는 catalogId/name/thumbnailUrl/imageUrl/source/version이 있다. 이미지의 독립적인 version을 도감 열기 시점에 보관하고 409에서 명시적 최신 조회 이후에만 바꾼다. 저장 성공 시 목록/상세를 무효화하며 검색·정렬·페이지·선택 대상은 유지한다.
- 도감 로딩/검색 결과 없음/오류/재시도를 구분한다. 저장 오류·충돌은 선택을 지우지 않는다. 공용 파일 조회 실패는 현재 테마 기본 그림을 거쳐 번들 중립 기본 그림으로 대체한다.
- 공개 `/demo`는 번들 기본 이미지만 사용하며 이미지 변경을 누르면 기존 읽기 전용 안내를 표시한다. 원문·추출 프롬프트·자동 이미지 매칭은 변경하지 않는다.

Backend 도감 원본/별칭/운영 등록 절차: `catchhole-backend-java/docs/world-image-catalog.md`.

장르별 초기 8장·기본 7장·공용 추천과 선택 보존은 [장르 이미지 연결](../world-image-themes.md)의 계약을 따른다.
