...
- 상세 API는 현재 설정 객체와 설정명별 최신 확정 후보 근거·후보 이력을 제공한다. 연결 근거가 없으면 FE는 직접 입력 설정으로 표시한다.
- Backend가 작품 전체와 동시 요청을 기준으로 `분류 + 대상명`, 대상 내 설정명 중복을 검증한다.
- 대상·설정 추가와 수정은 상세의 현재 `version`을 보내며, 409 충돌 시 FE는 입력을 유지하고 최신 상세만 다시 받아 재시도한다.
