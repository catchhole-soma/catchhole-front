...
- Hero: CatchHole 전용 에디토리얼 이미지를 섹션 전체 배경으로 사용하고 중앙 카피와 어두운 오버레이를 겹친다. `웹소설 원고에서 작품 설정까지` 배지, `원고 속 캐릭터와 세계관을, 근거와 함께 정리하세요` 제목, 서비스 설명, 로그인 없는 체험 주 버튼·무료 시작 보조 버튼, 원문 근거·작가 확정 신뢰 요소를 유지한다.
...
**5. 데이터 없음 / 실패 표시**
- 랜딩 카피·이미지·서비스 목록과 회원 탈퇴 성공 안내는 FE 정적 콘텐츠·라우트 상태이므로 별도의 로딩·빈 상태·API 실패 상태 없음
