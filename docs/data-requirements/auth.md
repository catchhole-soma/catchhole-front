...
- Hero: 넓은 화면에서는 카피와 CatchHole 전용 에디토리얼 이미지를 좌우로 분할하고, 좁은 화면에서는 카피 다음에 이미지를 배치한다. `웹소설 원고에서 작품 설정까지` 배지, `원고 속 캐릭터와 세계관을, 근거와 함께 정리하세요` 제목, 서비스 설명, 로그인 없는 체험 주 버튼·무료 시작 보조 버튼, 원문 근거·작가 확정 신뢰 요소를 유지한다.
