...
**6. BE와 협의할 범위·상태값**
- 6-1. 분석 상태·충돌 건수 뱃지를 카드에 표시할지부터 결정(FE는 제거 쪽 의견 — 집계 값이라 BE 부담 대비 효용 낮음). 표시하기로 하면 목록 응답 포함 vs 별도 집계 API 협의
- 6-2. "회차 수"는 `latestEpisodeNo`(가장 큰 회차 번호)로 확정 — 결번/분할 시 실제 개수와 어긋나는 표시를 허용할지만 협의
...
**6. BE와 협의할 범위·상태값**
- 6-1. 작품 생성(`POST /works`)과 첫 회차 업로드(`POST /works/{workId}/episodes`)는 분리 호출로 확정 — 작품만 생성되고 업로드가 실패한 경우의 사용자 흐름 협의
- 6-2. 장르를 자유 입력으로 둘지 고정 목록으로 둘지
...
**6. BE와 협의할 범위·상태값**
- 6-1. 대시보드 진입 시 작품 요약 집계(회차 수·충돌 수·분석 상태)를 한 번에 줄지
