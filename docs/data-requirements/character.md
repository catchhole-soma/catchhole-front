...
회차 업로드 후 하나의 업로드 묶음에 속한 회차별 설정 추출 Job들이 만든 후보를 원문 근거와 함께 검토하는 공통 화면. 이 문단은 `캐릭터 후보` 탭의 캐릭터 연결·설정값 보정·확정·무시 계약을 다루며, `세계관 후보` 탭은 [세계관 설정 요구사항](./world-setting.md#설정-후보-검토--세계관-후보-탭)을 따른다.
...
> **현재 구현 계약**
> - 기존 캐릭터 후보 저장·확정 로직과 상태 계약은 유지한다. 세계관 후보는 별도 데이터 계약을 사용하고 공통 검토 화면의 레이아웃·원문 근거·페이지네이션 패턴만 공유한다.
