...
> **사용자 용어**: 상위 사이드바는 `작품 설정`, 탭과 화면 제목은 `캐릭터 설정`으로 표시한다. 내부 URL의 `settingDB`와 API 모델 이름은 유지한다. 목록·상세·보관함은 한글 한 글자 원형 아바타 대신 텍스트 중심 식별 영역을 사용한다.

> **Theme V2 화면 계약**: 설정 DB 헤더·탭과 캐릭터 카드, 상세 모달, 변화 이력 패널, 원문 근거 패널은 밝은 surface와 파란 primary를 공유한다. 목록·상세·보관함의 로딩·빈 상태·오류는 공통 `database-state` 구조를 사용하고, 오류·삭제는 danger, 성공은 success 의미색을 유지한다. 현재 데이터 조회·페이지 계산·상세·이력·evidence API 계약은 테마 전환으로 변경하지 않는다.
...
> **Theme V2 화면 계약**: 상세는 흰색 모달, 이력은 우측의 흰색 floating panel, 원문 근거는 같은 패널을 대체하는 흰색 reader surface로 표시한다. 프로필·스킬 등 기존 보라색 유형 강조는 primary blue 계열로 옮기고, 나이·레벨·상태처럼 유형 구분에 필요한 보조 의미색은 유지한다. 원문 하이라이트는 연한 파란 배경과 본문색을 사용해 선택하지 않아도 읽을 수 있어야 한다.
...
> **Theme V2 화면 계약**: 파일 목록·선택 안내·전체 원문은 흰색 카드와 연한 회색 reader surface를 사용한다. 목록·원문 로딩, 빈 상태, 조회 오류는 같은 밝은 상태 패턴으로 교체하며, 업로드·삭제 모달도 공통 흰색 overlay shell을 사용한다. 선택·수정·삭제·업로드의 URL 및 API 계약은 유지한다.
...
- 선택한 파일 정보
  ...
  - 파일 형식
  - 파일 크기
- 설정집 단독 업로드는 AI 분석을 자동 시작하지 않으므로 외부 AI 처리 체크박스나 안내를 표시하지 않음
- 취소·업로드 버튼
...
> **Theme V2 화면 계약**: 검색 입력, 두 필터 묶음, 결과 카드는 흰색 surface와 파란 활성 상태를 사용하며, 검정 배경의 칩·카드는 사용하지 않는다. 캐릭터 상세에서 설정이 없는 영역은 큰 검정 박스 대신 아이콘·제목·설명이 있는 compact 빈 상태로 표시하고, 2열 설정에서 항목이 하나뿐이면 전체 폭을 사용한다.
...
> **Theme V2 화면 계약**: 캐릭터 후보 탭의 헤더·요약·필터·후보 그룹·상세·확정 액션과 수정·연결 모달을 밝은 검토 워크스페이스로 표시한다. 캐릭터·세계관 필터는 같은 높이·간격·파란 활성 상태를 사용하고 결과 없음 안내는 검은 패널 대신 밝은 공통 빈 상태를 사용한다. 기존값과 제안값은 각각 danger·success 계열의 옅은 배경 위에 본문 대비를 보장하며, 특히 비어 있지 않은 제안값과 확정·제외 후보의 읽기 전용 안내는 밝은 카드에서도 항상 본문색으로 읽혀야 한다. 후보 조회·필터 URL·수정·제외·연결·그룹 확정과 세계관 탭 전환 계약은 유지한다.
...
- 상단 분석·검토 요약
  - 분석 대상 회차 범위와 회차 수
  - 전체 후보 수
  - 반영됨·제외됨·직접 확인 수: 네 가지 필수 전체 집계 중 완료·제외·직접 확인을 각각 표시
  - 분석 중 수: 미확정 중 비교 대기·진행 항목과 해당 회차의 자동 반영 대기를 별도 상태로 표시
- 좌측 후보 그룹 목록 영역
  - 캐릭터명과 그룹 내 설정 수
  - 그룹의 출처 회차 목록
  - 연결 상태와 직접 확인·분석 중 개수
- 목록 필터
  - 검토 상태: 전체·미처리·반영됨·제외됨
...
| 확인 필요 수 | `attentionRequiredCandidateCount`, 0 이상의 정수·필수 | `PENDING_REVIEW`이면서 `AMBIGUOUS` 또는 비교 `FAILED`·`RECOMPARISON_REQUIRED`인 후보 수. 사유 중복 없이 배치 전체에서 집계 |
| 반영됨·제외됨·직접 확인·분석 중 | `confirmedCandidateCount`·`dismissedCandidateCount`·`directReviewCandidateCount`·`processingCandidateCount`, 각각 필수 정수 | 후보당 한 범주에 속하며 네 수의 합은 전체 수. 조회 필터·페이지 독립 |
...
- 유효하지 않은 scalar 수정 저장 후: canonical 표시값·`valueJson.value`, 갱신된 `valueValidation=VALID`, 재비교가 필요한 비교 상태
...
### Theme V2 상세·원문 표시 계약

- 캐릭터 후보와 설정 검색 상세의 `1차 추출 원문`, 캐릭터 상세·변화 이력의 전체 원문 패널은 검정 표면을 사용하지 않는다.
- 원문 근거는 `#F8FBFF` 계열의 밝은 표면, 블루 계열 위치 강조, 본문색 인용문을 공통으로 사용하며 quote·offset·회차 데이터는 변경하지 않는다.
- 상세·수정·연결·확인 모달은 흰색 surface, 공통 테두리·라운드·그림자와 흐린 배경을 사용한다. 로딩·빈 상태·오류·재시도는 동일한 본문 영역 안에서 교체한다.
- 캐릭터 수정의 동적 설정명 prefix와 이름·값 입력은 밝은 surface와 블루 강조를 사용한다. `프로필`·`스킬`·`아이템` 같은 prefix는 값의 일부가 아니라 canonical key 그룹을 설명하는 편집 보조 표시다.
- `STAT` schema의 한글 `displayName`은 정상 사용자용 명칭이고, `valueType=NUMBER`의 `value`는 숫자로 해석 가능한 문자열이어야 한다. FE는 응답값에 명칭이나 단위를 덧붙이지 않고 그대로 표시하며 숫자가 아닌 과거 저장값은 저장 시 숫자 검증에서 거절한다.
- 이 테마 계약은 표시 계층에만 적용하며 후보 확정, 캐릭터 연결, snapshot, 타임라인·evidence API 호출 경계를 변경하지 않는다.
...
> 자동 반영 보류 사유가 제공된 미확정 후보만 사유별 자연어 배지를 표시한다. 수정 저장은 직접 확인에서 빼지 않으며 확정 또는 제외 성공 후 서버 집계를 다시 조회한다. 캐릭터 비교가 현재 페이지 밖에서 진행되어도 전체 분석 중 수가 0이 될 때까지 목록과 다른 탭의 캐릭터 요약을 갱신한다.
...
### 순차 분석과 그룹 비교 통합 (2026-09-13)

Java #193의 `SettingCandidateResponse.analysisMode`로 검토 계약을 구분한다. 일반 후보는 #75의 공통 `comparisonRevision`과 비교 완료를 요구한다. 순차 후보는 회차별 입력으로 비교하므로 이 공통 revision 대상에서 제외하고 서버의 실제 상태·의존성 검증을 따른다. 자동 반영이 끝난 후보의 `manualReviewAvailable` 직접 수정→확정은 유지한다. mode가 생략된 응답에 순차 정책을 추정해서 적용하지 않는다.

후보 목록의 첫 대상 선택·페이지 보정은 재조회 완료 이후 수행한다. 응답 내용이 캐시와 같아도
`fetchStatus` 변경을 구독해 필터를 되돌린 뒤 선택이 빈 상태로 남지 않게 한다. 세계관 검토도 동일하다.
