...
- [설정DB 캐릭터 타임라인 탭](#설정db-캐릭터-타임라인-탭)
...
![설정DB - 캐릭터 타임라인 탭](../screens/Cguyz.png)

![캐릭터 설정 타임라인 기본 상태](../screens/FpNOR.png)

![캐릭터 설정 타임라인 원문 근거 상태](../screens/KMiGw.png)

작품에 확정 등록된 캐릭터를 선택하고, 해당 캐릭터에게 축적된 `CharacterFact` 전체 이력을 회차별로 확인하는 화면.

> **MVP 범위 메모**
> - 설정DB 탭에서 `캐릭터 타임라인`을 `캐릭터 DB`의 왼쪽에 배치한다.
> - 바깥 목록은 캐릭터를 고르는 진입점이며 이름과 첫 등장 회차만 표시한다. 나이·직업·현재 레벨 등 현재 대표값은 기존 캐릭터 DB에서 확인한다.
...
> **타임라인 필터와 Fact 유형 정책**
> - 요청 필터는 `ALL`, `PROFILE`, `AGE`, `LEVEL`, `STAT`, `SKILL`, `ITEM`, `STATUS`를 지원한다.
...
카드에는 나이, 직업, 레벨, 장르별 대표 속성을 표시하지 않는다. 기존 캐릭터 DB 목록의 서버 페이지네이션 규약은 재사용하되, 목록 배치는 타임라인 전용 compact 카드 규격을 사용한다.
...
- 왼쪽 회차 바로가기, 가운데 Fact 타임라인, 오른쪽 원문 근거 패널의 3열 구조
- 선택한 Fact 강조
...
**2. 사용자 액션**
- `캐릭터 타임라인` 탭 선택 → 캐릭터 타임라인 카드 목록 표시
- 캐릭터 카드 선택 → 해당 캐릭터의 타임라인 모달 열기
- 이전·다음 페이지 선택 → 화면에 맞춰 계산한 카드 수를 유지하며 캐릭터 목록 서버 페이지 이동
- 타임라인 모달 최초 진입 → 회차 바로가기는 선택하지 않고 가장 이른 회차의 첫 Fact부터 20개 조회
- Fact 유형 선택 → 선택한 유형으로 요약과 Fact 목록을 다시 조회하고 타임라인 목록·cursor·명시적 회차 선택을 초기화
- `전체` 선택 → 모든 지원 Fact 유형 표시
...
- Fact의 `근거` 선택 → 기존 캐릭터 DB와 동일한 CharacterFact 원문 근거 조회를 호출하고 오른쪽 패널을 열어 전체 원문을 기본 표시
- 원문 근거 패널에서 다른 Fact 선택 → 선택한 Fact의 근거로 교체
- 원문 근거 패널 닫기 → 선택 Fact만 해제하고 필터, 회차 범위, 로드한 Fact와 타임라인 스크롤 위치 유지
- `Escape` 입력 → 원문 패널이 열려 있으면 원문 패널을 먼저 닫고, 패널이 닫혀 있으면 타임라인 모달을 닫음
- 타임라인 모달 닫기 → 현재 캐릭터 목록 페이지를 유지한 채 탭 목록으로 복귀
- 다른 설정DB 탭 선택 → 선택한 탭으로 전환
...
**3. 화면 전환 식별자**
- 현재 작품: `workId`
- 현재 탭: `nav=settingDB&tab=timeline`
- 캐릭터 목록 페이지: `timelinePage={1 이상의 정수}`
- 타임라인 모달: `modal=character-timeline&charId={characterId}`
- Fact 유형 필터: `timelineFactType={ALL|PROFILE|AGE|LEVEL|STAT|SKILL|ITEM|STATUS}`
- 명시적으로 이동한 회차: `timelineEpisodeNo={1 이상의 정수}`
- 원문 근거 패널: `factId={characterFactId}`
- 타임라인 모달 URL 예시: `/dashboard?nav=settingDB&tab=timeline&timelinePage=1&modal=character-timeline&charId=01970c2e-7e6d-7000-8e5d-2a9bc4b6d111&timelineFactType=ALL`
- 회차 이동 URL 예시: `/dashboard?nav=settingDB&tab=timeline&timelinePage=1&modal=character-timeline&charId=01970c2e-7e6d-7000-8e5d-2a9bc4b6d111&timelineFactType=STATUS&timelineEpisodeNo=101`
- 원문 근거 URL 예시: `/dashboard?nav=settingDB&tab=timeline&timelinePage=1&modal=character-timeline&charId=01970c2e-7e6d-7000-8e5d-2a9bc4b6d111&timelineFactType=STATUS&timelineEpisodeNo=101&factId=01970c2e-7e6d-7000-8e5d-2a9bc4b6d222`
- 서버가 반환한 `nextCursor`, 현재까지 로드한 페이지 수와 스크롤 좌표는 일시적인 조회 상태이므로 URL에 저장하지 않는다.
- 새로고침 시 `timelineFactType`과 `timelineEpisodeNo`를 기준으로 첫 묶음부터 다시 조회한다.
- 원문 근거 패널을 닫을 때는 `factId`만 제거한다.
- 타임라인 모달을 닫을 때는 `modal`, `charId`, `timelineFactType`, `timelineEpisodeNo`, `factId`를 제거하고 `timelinePage`는 유지한다.

**4. 데이터 없음 / 실패 표시**
- 캐릭터 목록 조회 중 로딩
- 등록된 활성 캐릭터가 없으면 기존 캐릭터 DB 빈 상태와 같은 원고 분석 안내 제공
- 캐릭터 목록 조회 실패 시 오류 안내와 다시 시도 제공
- 첫 등장 회차가 없으면 `첫 등장 —`로 표시
...
**5-1. BE → FE 제공 데이터 요구사항 — 캐릭터 선택 목록**

- 기존 캐릭터 DB의 `ACTIVE` 캐릭터 서버 페이지를 재사용한다.
- 타임라인 카드에서는 기존 응답 중 캐릭터 식별자, 이름, 첫 등장 회차 번호만 사용한다.
- 나이·장르별 대표 속성 등 기존 응답의 다른 값은 타임라인 카드에 표시하지 않는다.
- 목록 정렬, 페이지 메타데이터와 권한 정책은 캐릭터 DB 목록과 동일하게 유지한다.

**5-1. BE → FE 제공 데이터 요구사항 — 타임라인 요약**
...
- 회차 바로가기 목록과 현재 필터의 Fact 개수는 요청한 유형 필터를 적용한 결과다.
...
- 현재 MVP cursor는 캐릭터·필터·시작 회차와 다음 조회 offset을 담는다. 정렬용 근거 offset과 생성 시각은 목록 항목 필드로 노출하지 않는다.
- cursor는 URL-safe 불투명 문자열로 제공한다. FE는 내용을 해석·수정하지 않고 다음 요청에 그대로 전달한다.
- cursor는 캐릭터, 유형 필터와 정렬 경계가 일치할 때만 유효하다. 다른 캐릭터·필터의 cursor를 전달하면 400으로 거절한다.
- 동일 회차 Fact가 두 cursor 묶음에 걸칠 수 있다. 서버는 정렬을 유지하고 FE는 `characterFactId`로 중복 제거한 뒤 동일 회차 그룹에 병합한다.
- 목록 응답에는 `factKey`, `isCurrent`, `effectiveFromEpisodeNo`, `valueJson`, `normalizedValue`, 전체 원문, `evidenceSpans`, raw AI 응답을 포함하지 않는다.
...
| 캐릭터 목록 page | 0 이상의 정수·필수 | 목록 조회 시 URL의 1-based `timelinePage`를 0-based로 변환 |
| 캐릭터 목록 size | 1~24 정수·필수 | 화면 맞춤 카드 개수 |
...
| --- | --- | --- | --- |
| 모달 최초 진입 | 캐릭터 ID, `ALL`, size 20 | 요약 + 첫 Fact 묶음 | 선택 없는 `1–10화` 범위와 가장 이른 Fact 표시 |
...
| 원문 근거 열기 | 작품 ID, CharacterFact ID | 기존 CharacterFact 원문 근거 | 오른쪽 패널에서 전체 원문·하이라이트 또는 quote fallback 표시 |

**권장 계약 경계와 생성 타입**

- 기존 캐릭터 목록 계약 `getCharacters`를 재사용한다.
...
- 요약 query key에는 `workId`, `characterId`, `factType`을 포함한다.
- Fact infinite query key에는 `workId`, `characterId`, `factType`, `fromEpisodeNo|null`을 포함한다.
...
- 캐릭터 선택 카드는 `아바타 · 이름 · 첫 등장 회차 · 이동 아이콘`을 한 줄에 표시하고 높이를 `64px`로 고정한다.
- 카드 최소 너비는 `220px`, 열 간격은 `14px`로 하며, 기준 데스크톱 콘텐츠 폭 `980px`에서는 4열(`234.5px × 4`)로 배치한다.
- 화면 폭이 줄어들면 3열·2열·1열로 자연스럽게 감소하며, 데스크톱에서도 최대 4열을 넘기지 않는다.
- 타임라인 카드의 고정 높이를 기준으로 완전히 보이는 행만 페이지 크기에 포함하고 최대 요청 크기는 24개로 유지한다.
- 목록 영역 크기 변경 시 `ResizeObserver`로 열·행·요청 크기를 다시 계산한다.
- 페이지 크기가 바뀌면 기존 첫 항목의 절대 순번을 새 크기의 페이지로 환산해 같은 항목이 포함된 범위를 유지한다.
- 모달은 한 화면에서 독립적으로 스크롤하며 바깥 캐릭터 목록의 페이지 계산에 영향을 주지 않는다.

**타임라인 모달**

- 데스크톱 기본 상태는 회차 바로가기와 Fact 타임라인의 2열 구조다.
- 원문 근거가 열리면 회차 바로가기, Fact 타임라인, 전체 원문의 3열 구조로 바꾼다.
- 원문 패널이 닫힌 상태에서 오른쪽 빈 영역을 미리 확보하지 않는다. Fact 타임라인이 사용 가능한 너비를 모두 사용한다.
- 768~1199px에서는 회차 바로가기를 접을 수 있게 하고 원문 근거를 우측 overlay drawer로 표시한다.
- 768px 미만에서는 모달을 전체 화면으로 표시하고 회차 바로가기는 검색 가능한 dropdown, 원문 근거는 타임라인을 덮는 전체 화면 하위 상태로 표시한다.
...
> 관계도·등장 장면·작품 사건 타임라인을 캐릭터 상세에 함께 제공하는 형태는 MVP 이후 고도화 범위다. CharacterFact 전체 이력은 별도의 [설정DB 캐릭터 타임라인 탭](#설정db-캐릭터-타임라인-탭)에서 다룬다.
...
> - 과거 설정 이력은 이 모달에 표시하지 않고 [설정DB 캐릭터 타임라인 탭](#설정db-캐릭터-타임라인-탭), [설정DB 검색 탭](#설정db-검색-탭)과 [설정DB 검색 결과 상세 모달](#설정db-검색-결과-상세-모달)에서 조회한다.
