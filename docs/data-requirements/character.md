...
> **1차 조회 계약**
> - 이번 구현 단위는 `batchId` 회차 범위, 전체·완료·대기·연결 필요 집계, 검토·연결 상태 필터, 서버 페이지네이션과 후보 목록·상세 조회까지 연결한다.
> - 설정 유형 분류·필터, 상태별 세부 집계, 사용자 수정 여부, 목록·상세 DTO 분리는 후속 조회 단위에서 보강한다.
> - 후보 수정·캐릭터 연결·확정·무시와 검토 완료 후 다음 단계 분기는 후속 변경 단위에서 연결하며, 그전까지 화면 변경 버튼은 비활성화한다.
