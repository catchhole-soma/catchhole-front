...
> - 상세 모달의 `삭제`는 MVP에서 지원하되 실제 데이터 삭제가 아니라 `ACTIVE → ARCHIVED` 상태 전환으로 처리한다. 보관된 캐릭터 목록·복구 화면은 MVP 이후 지원한다.
...
> - 프로필은 저장된 항목을 `표시명: 값` 행으로 표시하고, 스탯은 같은 형식의 항목을 가로 2열로 배치해 한 줄에 최대 2개씩 표시한다.
...
- 편집 중 프로필·스탯·스킬·아이템·상태의 새 항목 추가. 프로필·스탯은 표시명과 값을 받고, 스킬·아이템·상태는 이름과 세부 속성을 함께 편집한다.
