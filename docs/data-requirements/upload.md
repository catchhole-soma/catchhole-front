...
> **AI 원고 처리 안내**: 실제 AI 분석을 시작하는 화면에는 `AI 분석을 실행하면 원고의 필요한 구간이 OpenAI API로 처리됩니다.`와 `API 입력·출력은 기본적으로 모델 학습에 사용되지 않습니다. 자세히 보기`를 체크박스 없이 표시한다. 자세히 보기는 개인정보 처리방침의 외부 AI 처리 항목을 연다.
