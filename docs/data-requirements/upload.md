...
- 회차별 분석 Job 집계 상태
  ...
  - 모든 Job의 `SUCCEEDED`: 업로드 묶음 분석 성공
...
- 헤더 `뒤로` 또는 다른 화면으로 이동
  ...
  - [원고 목록](./episode.md#원고-목록-대시보드-원고-탭)에서 분석 중인 회차의 `진행 보기`나 분석 실패 회차의 `다시 시도`를 눌러 언제든 이 화면으로 재진입
...
| 분석 작업 식별 정보 | 회차별 목록·필수 | 모든 Job polling, 새로고침 복원과 원고 목록 재진입 대상 |
...
- **현재 계약**: 별도 `PARTIAL_FAILED` 없이 FE가 현재 Job 목록에서 활성 작업 여부, `SUCCEEDED`, `FAILED` 조합을 집계
