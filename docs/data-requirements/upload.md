...
| `content` | 목록 항목별 문자열·필수 | 서버가 감지 경계로 분리한 원문이며 미리보기에 사용 |
...
- **BE·FE 현재 계약**: 감지 API는 원고를 저장하지 않고 정규식 기반 결과를 `detectionOrder`, `sourceFileIndex`, 번호, 제목, `content`, 글자 수로 반환
...
| `content` | 목록 항목별 문자열·필수 | 서버가 감지 경계로 분리한 원문 미리보기 |
| 글자 수 | 목록 항목별 0 이상의 정수·필수 | 해당 감지 회차 원문의 공백 제외 Unicode 문자 수 |
