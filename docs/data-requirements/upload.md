...
> **AI 원고 처리 고지**: 원고의 외부 AI 처리는 회원가입 때 확인하는 현재 개인정보처리방침에 고지한다. 가입 후 업로드·재시도·재분석 화면에서는 이를 별도 동의처럼 반복 노출하거나 재동의를 요구하지 않는다. 상세 내용은 공개 개인정보처리방침에서 언제든 확인할 수 있다.

> 회차·설정집 저장과 회차 파일 교체 API에는 정책 버전이나 동의 필드를 전달하지 않는다. 설정집 단독 업로드와 회차 파일 교체는 AI 분석을 자동 시작하지 않으므로 AI 처리 안내를 표시하지 않으며, 회차 재분석을 실제 요청하는 확인창에서 다시 안내한다.
...
작품별 `GET /api/v1/works/{workId}/episodes/upload-policy`에서 미확정 캐릭터·세계관 후보 수와 전체 원고 제한을 조회한다. 선행 단일 회차 개수에 따른 다회차 조건은 없으며 두 다회차 카드를 항상 제공한다. 단일 회차 카드는 추천을 유지한다. 이 조회는 업로드 자격이나 자동 반영 사용 여부를 판단하지 않으며, 로딩·실패 중에도 모든 업로드 방식과 반영 선택을 사용할 수 있다. 조회 실패는 미확정 정보 안내를 다시 확인하는 보조 동선으로 표시한다.

작품 전체에 검토 대기 후보가 있으면 두 종류의 개수와 먼저 검토·확정하면 분석에 도움이 된다는 안내를 표시한다. 이 안내는 업로드를 차단하지 않는다. 분석 목록으로 이동해 기존 결과를 확인할 수 있다.

다회차는 선택지 없이 회차 번호 순서대로 분석하고, 판단이 명확한 캐릭터·세계관 설정을 자동 저장한 뒤 다음 회차를 진행한다. 생성 요청은 `analysisMode=ORDERED_PROVISIONAL`, `reviewMode=AUTOMATIC`이다. 확정 설정과 앞 회차의 미확정 제안은 구분해 참고한다. 모호하거나 비교에 실패한 설정은 검토 대기로 남고, 결과 화면은 기존 `PENDING_REVIEW` 기본 필터로 이 항목만 먼저 표시한다. 자동 저장 결과는 확정/전체 필터와 작품 설정에서 확인할 수 있다.

단일 회차는 `AI 판단으로 설정 자동 반영`이 기본 선택이며 다회차와 같은 ordered+automatic 요청을 사용한다. 사용자가 `모든 설정 직접 검토`를 선택하면 `reviewMode=MANUAL`과 기존 분석 모드 생략 계약으로 전체 결과를 직접 확인할 수 있다. 다른 업로드 방식으로 바꾸었다 돌아오면 자동 반영을 기본으로 복원한다.

원고 목록에서 파일을 교체한 뒤 실행하는 단일 회차 재분석은 새 원고 업로드와 구분한다. 기존 확정 설정을 자동 변경하지 않는 확인창 안내에 맞춰 `SETTING_EXTRACTION` 요청에 해당 `episodeId`와 `reviewMode=MANUAL`을 명시한다.

파일 교체만으로 분석을 실행하지 않는다. 변경한 회차만 재분석하며 이미 완료된 후속 회차 결과는 유지한다. 재분석 후보를 직접 확정할 때 뒤 회차의 같은 항목과 직접 수정한 현재값은 보호하고 이력에 남긴다. 그 외에는 선택한 반영 방식에 따라 현재값에도 반영할 수 있다. 실제 반영 결과는 서버의 `historyOnly`로 표시한다.

단일·다회차 모두 선택한 **회차 원고 전체의 공백 포함 Unicode 문자 수 합계 250,000자까지** 허용한다. 정확히 250,000자는 허용하고 250,001자는 감지 또는 최종 업로드 단계에서 거부한다. 파일 용량 제한(10MB)과 별개이며 설정집 원본은 회차 원고 합계에 포함하지 않는다. 감지 응답 `totalUploadCharacters`를 표시하고, 기존 공백 제외 `charCount`는 회차별 기존 표시용으로만 사용한다. 서버의 분량 초과 감지 오류가 발생하면 회차 번호를 직접 입력하더라도 최종 업로드 버튼을 잠근다.

순차 분석의 앞 회차가 실패하거나 비교 미완료이면 뒤 회차는 `앞 회차 재개 대기`로 표시한다. 이 대기 때문에 재개 버튼을 잠그지 않고 `중단된 회차부터 재개`를 제공한다. 완료된 추출·비교는 보존하며 응답의 Job ID가 같아도 상태 캐시를 즉시 갱신하고 재조회한다. 성공 상태여도 변경 기록 또는 자동 저장 완료 시각이 없으면 완료·검토 화면을 열지 않는다. 분석 목록의 진행·완료 집계도 같은 기준을 사용한다. 자동 반영 뒤 남은 확인 대기 후보는 다음 회차를 막지 않으며 목록에서는 `분석 완료 · N개 확인 가능`으로 구분한다.

원문·검토 변경으로 실행이 무효화되면 `변경된 내용으로 새 분석이 필요합니다`와 `원고 목록에서 확인`을 제공한다. 기존 실행의 재개와 후보 검토는 막고 새 분석을 자동 요청하지 않는다. 내부 무효화 사유는 노출하지 않는다. 모든 회차 성공과 변경 기록 완료를 확인한 뒤 후보 검토로 이동한다. 일반 분석의 기존 부분 실패·토큰 중단 정책은 유지한다.

재시도로 교체된 이전 작업 ID는 URL 추적 이력에 남기되 반복 조회는 현재 작업만 한다. 새 순차 분석은 현재 작업 전체의 상태 응답을 받은 뒤 진행 중인 작업이 없을 때만 허용한다. 확인창을 연 뒤 진행 상태가 바뀌면 제출도 같은 조건으로 잠그며 취소는 허용한다.

`새 순차 분석`을 누르면 업로드 묶음에서 현재 보관 중인 회차 전체의 최신 목록·개수와 AI 사용량 재차감을 확인한다. 원래 실행의 일부 회차나 뒤 회차만 재분석하는 기능은 아니다. 확인창에서 `새 순차 분석 시작`을 눌렀을 때만 batch 전체 ordered 생성 API를 호출하며, 새 Job ID로 현재 화면을 전환하고 이전 Job ID는 추적 이력에 보존한다. 해당 묶음에 아직 진행 중인 작업이 있으면 요청을 잠그거나 서버 충돌 안내를 표시한다. 대상 첫 회차부터의 내용을 이미 확정하여 과거 시작 상태를 구성할 수 없으면 서버의 제한 메시지를 유지한다. 자동 기본 모드 전환이나 자동 재시도는 하지 않는다.
...
- `단일 회차 업로드` 카드 왼쪽 상단에 `추천` 배지를 표시하고, 카드 안에 `가장 정확한 설정 분석을 위해 한 회차씩 업로드하는 것을 권장해요.` 안내를 표시
- 추천은 설정 분석 정확도를 기준으로 한 업로드 방식 안내다. 다회차 파일의 회차 분리 오류를 의미하지 않으며, 세 업로드 방식은 모두 선택 가능
- 방식 선택과 각 입력 단계에서 업로드 방식 카드가 보일 때 같은 추천 배지·안내를 유지
...
업로드 방식 카드의 명칭·설명·추천 배지·권장 안내는 FE 정적 데이터이며 BE 제공 대상이 아니다.
...
- 원문 순서의 회차별 진행 카드
  ...
  - 회차 처리 상태와 상태별 단계 표시
  - `UPLOADED`: 원문 저장 완료
  - `CHUNKING` / `CHUNKED`: 원문 나누는 중 / 원문 정리 완료
  - `PREPROCESSING` / `PREPROCESSED`: 내용 파악 중 / 내용 파악 완료
...
- 미확정 설정 안내는 업로드 방식 카드 아래 20px 간격을 두어 독립된 안내 영역으로 구분한다.

검증 화면: [데스크톱](../screens/gh180-upload-spacing-1280.png) · [320px 모바일](../screens/gh180-upload-spacing-320.png). 두 화면 모두 카드와 안내 사이 20px, 가로 넘침 없음.
