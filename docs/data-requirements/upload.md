...
- 분석 성공 후에는 자동 이동하지 않고 완료 상태와 목적별 버튼을 표시
  ...
  - `설정 후보 검토` → 현재 업로드의 `workId`, `batchId`를 유지해 [설정 검토](./character.md#설정-검토-ssettingreview)로 이동
- 회차 일부 또는 전체 분석 실패
...
- **BE·설정 DB**: 기존 설정 구축 작업 성공 시 같은 `batchId`의 설정 후보 검토로 이동하고, 후보 확정 결과의 설정 DB 반영과 검토 완료 상태를 어떻게 연결할지
