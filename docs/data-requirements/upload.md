...
- **팀·디자인**: Pencil 노드 `Yr63L`에서 아래 상태 보강 필요
  - 화면 부제를 `회차 정보를 입력하세요`로 변경하고 별도 설정집 분석 결과 단계를 포함하지 않는 MVP 진행 단계
  - 설정집 옵션을 선택한 경우에만 설정집 파일 영역 노출
  - 번호·제목 자동 감지와 감지 실패·충돌, 중복 번호, 파일 검증 상태
  - 필수값에 따른 분석 시작 버튼 활성·비활성, 제출 중, 부분 성공과 실패 작업 재시도 상태
- **FE 보강**: 현재 mock 기반 회차·분석 생성을 실제 저장·분석 흐름으로 교체하고, 작품별로 하드코딩된 업로드 목적 기본값과 파일 미선택 시 데모 모드 전환을 제거
- **팀·문서**: 상단 공통 흐름과 `4. 설정집 분석 결과 확인`은 해당 범위를 검토할 때 설정집 원본만 저장하는 MVP 흐름과 맞게 정리
...
- **팀·디자인**: Pencil 노드 `geQNP`에서 아래 상태 보강 필요
  - 화면명·미리보기의 `AI` 표현과 두 분리 기준 선택 제거
  - 진행 단계를 `업로드 방식 → 원고 파일 입력 → 회차 분리 확인 → 분석 진행`으로 변경
  - 설정집 옵션을 선택한 경우에만 설정집 파일 영역을 노출하고 설정집 분석 결과 단계를 MVP 진행 단계에서 제외
  - 파일 선택 전, 감지 중, 감지 완료, 파일 수정 필요, 재시도 가능 실패 상태 분리
  - 현재 파일의 정상 감지 결과가 있을 때만 다음 버튼 활성화
...
- **팀·디자인**: Pencil 노드 `uBFUN`에서 아래 상태 보강 필요
  - 시작 회차 번호와 파일 개수 입력 제거, 선택한 실제 파일 수 표시
  - 회차·설정집 파일 안내를 TXT 전용으로 변경
  - 파일별 원본 파일명·크기·번호 후보·최종 번호·선택 제목을 확인·수정하는 매핑 목록 추가
  - 감지 중, 자동 감지 완료, 번호 충돌, 번호 미감지, 제목 미감지, 파일 읽기 실패, 묶음·기존 회차 번호 중복 상태
  - 설정집 옵션을 선택한 경우에만 설정집 파일 영역을 노출하고 설정집 분석 결과 단계를 MVP 진행 단계에서 제외
  - 모든 회차 파일의 최종 번호가 유효하고 중복되지 않을 때만 분석 시작 버튼 활성화
- **FE 보강**: 현재 시작 번호와 수동 파일 개수로 mock 회차를 만드는 흐름을 선택 파일별 감지·보정 목록과 실제 묶음 저장으로 교체하고, 파일 미선택 시 데모 모드로 분석 단계에 진입하는 흐름 제거
- **팀·문서**: 2A·2B의 TXT/DOCX 안내와 상단 공통 흐름·`4. 설정집 분석 결과 확인`은 각 해당 범위를 다시 검토할 때 MVP의 TXT 전용·설정집 원본 저장 흐름에 맞게 정리
...
- **팀·디자인**: Pencil 노드 `lTe6i`에서 아래 상태 보강 필요
  - 진행 단계를 `업로드 방식 → 원고 파일 입력 → 회차 분리 확인 → 분석 진행`으로 변경
  - `AI가 감지`, `자동 감지` 표현을 명시적인 회차 제목 행 기준 감지로 변경
  - 회차 포함 토글, 삭제, 합치기, 나누기, 분리 지점 선택과 `다시 분리` 액션 제거
  - 회차 번호·제목 수정과 번호·제목 검증 오류 상태 유지
  - 설정집 원본을 함께 올려도 설정집 분석 결과 단계를 추가하지 않고 확정 후 분석 진행으로 이동
  - 임시 결과 문맥 없음, 원문 미리보기 실패, 확정 중과 확정 실패 상태 보강
...
- 분석 성공 후에는 자동 이동하지 않고 완료 상태와 목적별 버튼을 표시
  ...
  - `설정 DB 보기` → 기존 설정 구축 결과가 반영된 [설정 DB](./character.md#설정db-캐릭터-탭)로 이동
- 회차 일부 또는 전체 분석 실패
...
| 기존 설정 구축 반영 상태 | 단일 상태값·기존 설정 구축 성공 후 필수 | 설정 DB 반영 완료 여부를 확인하고 반영 완료 후 설정 DB 이동 허용 |
...
- **BE·설정 DB**: 기존 설정 구축 작업 성공 시 설정 검토를 강제하지 않고 추출 결과를 설정 DB에 반영하며, 반영 완료 상태를 분석 작업 성공과 어떻게 연결할지
- **팀·디자인**: Pencil 노드 `zbVd6`에서 아래 변경 필요
  - MVP 진행 단계에서 `설정집 분석 결과`를 제거하고 업로드 방식별 실제 단계 표시
  - `회차 생성 및 분석 중`을 이미 생성된 회차의 분석 작업을 추적하는 문구로 변경
  - 목적과 무관한 `AI 설정 추출` 문구를 회차 처리 상태의 공통 `AI 분석` 문구로 변경
  - 분석 성공, 일부·전체 실패, 상태 조회 실패, 재시도 중과 목적별 결과 버튼 상태 보강
  - 화면 이탈 후 원고 목록에서 재진입하는 상태 보강
