...
**2. 사용자 액션**
- 화면 진입·탭 복귀 시 최신 분석 상태 조회
- 진행 중에는 상태를 주기적으로 갱신하고, 완료·실패가 확정되거나 화면을 벗어나면 갱신 중단
- 완료 후 자동 이동하지 않고 사용자가 `결과 보기`를 선택해 오류 리포트로 이동
- 화면 이탈은 분석 취소가 아니며 다시 진입하면 현재 작업 상태를 복원

**3. 화면 전환 식별자**
- `analysisJobId`, `episodeId`, 완료 결과 식별자 → 사용자가 `결과 보기`를 선택하면 [오류 리포트](#오류-리포트-s5report)로 이동
- 회차 업로드 쪽은 이 화면을 거치지 않고 자체 진행 단계에서 분기 — [회차 업로드의 분석 진행](./upload.md#5-분석-진행) 참고
...
회차 원고와 기존 설정 DB를 대조해 충돌/모순을 보여준다. 대시보드에서 분석을 실행했을 때 사용자가 결과 보기를 선택해 도착하는 화면으로, 단일 회차 또는 `?mode=prePublish`로 작품 전체 회차를 대상으로 한다. ([회차 검사 결과](#회차-검사-결과-sepisodevalidationreport)와 카드 구조는 같고, 대상 범위·진입 경로만 다름)
...
**2. 사용자 액션**
- 오류 무시 / 되돌리기, 카드 펼치기
- 수정 제안 복사, 원문 보기(읽기 전용), 원고 목록으로 이동
- 원문 수정이 필요하면 원본 파일에서 수정한 뒤 원고 목록의 `회차 파일 변경`으로 교체
- 공유, 발행 전 전체 검수 모드 전환
