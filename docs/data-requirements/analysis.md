...
> 이 문서는 `EPISODE_VALIDATION` 분석의 진행 상태와 충돌 의심 결과 조회 흐름을 정의한다. 현재 Java BE에는 `AnalysisJob` 생성·목록·상세 조회와 작업 상태 전이 계약이 있고, Python Worker에는 설정 추출 흐름이 구현되어 있다. 회차별 검증 진행 상태와 충돌 의심 결과 저장·조회 API는 후속 구현이 필요하다.
>
...
- [분석 리포트 목록 (S5ReportList)](#분석-리포트-목록-s5reportlist)
...
| 회차별 실패 요약 | Job `errorMessage`·선택 | 실패가 아니면 `null` |
...
- `분석 내역으로 돌아가기` → [분석 리포트 목록](#분석-리포트-목록-s5reportlist)으로 이동
...
- 충돌 의심 결과가 0건이면
  ...
  - 충돌 검토 버튼 대신 `분석 내역으로 돌아가기`를 주요 후속 액션으로 제공
...
> - 완료·실패 분석 작업을 덮어쓰거나 삭제하지 않는다.
> - 기본 정렬은 분석 작업 생성 시각 내림차순이다.
> - 성공 결과는 분석 당시 사용한 원고·확정 설정 기준과 함께 보존한다. 저장 이후 바뀐 기준과 비교하여 결과 상태를 다시 판정하는 기능은 MVP에 포함하지 않는다.
> - MVP에서는 사용자가 이력을 삭제하는 기능을 제공하지 않는다.
...
- 실패 실행 묶음 선택 → 해당 `analysisJobIds`의 `/loading` 실패 상태에서 실패 회차 요약과 실행 시각 확인
...
- 분석 작업 목록 조회 중 로딩
- 분석 작업 이력이 없으면 `아직 분석 내역이 없습니다.` 표시
...
**5-2. BE → FE 제공 데이터 요구사항 — 작업 목록 상태·결과 요약 정보**

| 데이터 의미 | 형태·필수성 | 값 없음·조건 |
| --- | --- | --- |
| 분석 작업 식별자 | 목록 항목별 단일 값·필수 | 없음 |
| 결과 식별자 | 단일 값·선택 | `SUCCEEDED`일 때 필수, 그 외 상태에서는 `null` |
| 분석 대상 회차 범위·회차 수 | 목록 항목별 값·필수 | 대상이 없으면 `0` |
| 작업 상태 | enum·필수 | `PENDING`/`RUNNING`/`SUCCEEDED`/`FAILED` |
| 현재 단계 | enum 또는 문자열·선택 | 대기 전·완료·실패 후 `null` 가능 |
| 상태별 회차 수 | 상태별 정수·선택 | 진행 집계 미제공 시 `null`, 제공 시 없는 상태는 `0` |
| 전체 충돌 의심 건수 | 0 이상의 정수·결과가 있으면 필수 | 없으면 `0` |
...
기존 분석 작업 목록 응답을 화면 요약 정보까지 확장하는 방향을 우선 사용한다.
...
- 목록 응답에 회차별 상태 집계를 포함할지, 진행 작업에 한해 별도 상세 조회를 사용할지
- 대기·진행 중 작업이 있을 때 목록 갱신 주기와 서버 부하 기준
- 재시도 원본과 새 분석 작업의 연결을 어떤 응답 필드로 제공할지
- 분석 기준 요약에 모델명, 확정 설정 기준, 원고 버전 중 어디까지 사용자에게 표시할지
...
- 분석 리포트 목록에서 선택한 완료 결과도 동일하게 조회하고 검토 상태 변경
