...
> 사이드바 하단은 API의 `remainingPercent`만 `남은 사용량`으로 표시합니다. 이 비율은 누적 지급량이 아니라 현재 1회 제공량을 100% 기준으로 계산하므로, 소진 뒤 최초 제공량과 같은 양을 추가 지급받으면 100%로 복구됩니다. 정확한 token 수와 처리 중 예약량은 사용자에게 노출하지 않습니다. 분석 생성·재시도에서 `AI_TOKEN_QUOTA_EXHAUSTED` 응답을 받으면 요청을 시작한 확인 모달을 닫고 전역 안내 모달을 연 뒤 미처리 추가 사용량 요청부터 조회합니다. 미처리 요청이 없을 때만 35~1000자 피드백 입력과 `제출`·`취소`를 제공하며, 제출한 뒤에는 운영자 검토 중 상태로 전환합니다. 운영자는 별도 Backend API에서 수동 승인·반려하고, 승인 시 신규 회원 초기 지급량과 같은 설정값을 추가합니다. 누적 지급량과 사용량은 초기화하지 않습니다.
...
```mermaid
flowchart TD
  trigger["상단 의견 보내기"] --> form["35~1000자 의견 입력"]
