...
> **값 검증 분기**: 캐릭터 row의 `valueValidation=INVALID`면 경고를 표시하고 수정·제외은 허용하되 그룹 확정을 잠그다. NUMBER/BOOLEAN 수정값은 모달에서 먼저 검증하고, 저장 후 서버가 내려준 새 `valueValidation`이 유효해야 확정 동선으로 돌아간다.
