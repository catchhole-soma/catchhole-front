...
아래 표의 경로와 각 다이어그램 밑의 딥링크는 배포 사이트 <https://www.catchhole.com> 로 연결되는 **실제 클릭 가능한 링크**입니다. 단,
...
| --- | --- | --- |
| 랜딩 | [`/landing`](https://www.catchhole.com/landing) | 로그인 전 서비스 소개 페이지 |
| 로그인 / 회원가입 | [`/login`](https://www.catchhole.com/login) · [`/signup`](https://www.catchhole.com/signup) | 인증 |
| 작품 선택 | [`/works`](https://www.catchhole.com/works) | 작업할 작품을 고르는 진입점 |
| 대시보드 | [`/dashboard`](https://www.catchhole.com/dashboard) | 작품의 설정DB·리포트·그래프·원고 허브 |
| 원고 에디터 | [`/editor`](https://www.catchhole.com/editor) | 회차 원고 편집/보기 |
| AI 챗봇 | [`/chat`](https://www.catchhole.com/chat) | 설정 관련 질의응답 챗봇 |
| 분석 진행 중 | [`/loading`](https://www.catchhole.com/loading) | 분석 진행률 표시(완료 시 리포트로 자동 이동) |
| 충돌·모순 리포트 | [`/report`](https://www.catchhole.com/report) | 분석 결과(충돌/모순) 리포트 |
| 회차 업로드 | [`/episode-upload`](https://www.catchhole.com/episode-upload) | 회차 원고 업로드 플로우 |
| **설정 후보 검토** | [`/setting-review`](https://www.catchhole.com/setting-review) | AI가 원고·설정집에서 **뽑아낸 설정 후보**를 작가가 확인·확정 |
| **회차 검사 결과** | [`/episode-validation-report`](https://www.catchhole.com/episode-validation-report) | 새로 올린 회차가 **기존 설정과 충돌·모순**되는지 검사한 결과 |
...
> 딥링크: 약관·개인정보 모달을 바로 열기 — [`/login?terms=terms`](https://www.catchhole.com/login?terms=terms) · [`/login?terms=privacy`](https://www.catchhole.com/login?terms=privacy) (회원가입은 [`/signup?terms=terms`](https://www.catchhole.com/signup?terms=terms) · [`/signup?terms=privacy`](https://www.catchhole.com/signup?terms=privacy)).
...
> 리포트는 **단일 회차 검수**([`/report`](https://www.catchhole.com/report)) / **발행 전 전체 검수**([`/report?mode=prePublish`](https://www.catchhole.com/report?mode=prePublish)) 두 모드가 있습니다.
...
> 딥링크 (클릭 시 이동):
> - 사이드바 — [설정 DB](https://www.catchhole.com/dashboard?nav=settingDB) · [분석 리포트](https://www.catchhole.com/dashboard?nav=reports) · [그래프 뷰](https://www.catchhole.com/dashboard?nav=graph) · [원고 목록](https://www.catchhole.com/dashboard?nav=manuscripts)
> - 설정DB 탭 — [캐릭터](https://www.catchhole.com/dashboard?nav=settingDB&tab=characters) · [관계도](https://www.catchhole.com/dashboard?nav=settingDB&tab=relations) · [타임라인](https://www.catchhole.com/dashboard?nav=settingDB&tab=timeline) · [세계관 규칙](https://www.catchhole.com/dashboard?nav=settingDB&tab=worldrules) · [검색](https://www.catchhole.com/dashboard?nav=settingDB&tab=search)
> - 관계도 샘플 — [triangle](https://www.catchhole.com/dashboard?nav=settingDB&tab=relations&relGraph=triangle) · [prosecution](https://www.catchhole.com/dashboard?nav=settingDB&tab=relations&relGraph=prosecution) · [court](https://www.catchhole.com/dashboard?nav=settingDB&tab=relations&relGraph=court)
...
> 딥링크 (클릭 시 이동): 리포트 [발행 전 검수](https://www.catchhole.com/report?mode=prePublish).
> ID 필요(형식만): 설정 후보 검토 `?candidate=<id>` ([/setting-review](https://www.catchhole.com/setting-review)), 회차 검사 결과 `?issue=<id>` ([/episode-validation-report](https://www.catchhole.com/episode-validation-report)).
