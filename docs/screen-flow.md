...
| 화면 이름 | 경로 | 무슨 화면인가 |
| --- | --- | --- |
| 랜딩 | `/landing` | 로그인 전 서비스 소개 페이지 |
| 로그인 / 회원가입 | `/login`, `/signup` | 인증 |
| 작품 선택 | `/works` | 작업할 작품을 고르는 진입점 |
| 대시보드 | `/dashboard` | 작품의 설정DB·리포트·그래프·원고 허브 |
| 원고 에디터 | `/editor` | 회차 원고 편집/보기 |
| AI 챗봇 | `/chat` | 설정 관련 질의응답 챗봇 |
| 분석 진행 중 | `/loading` | 분석 진행률 표시(완료 시 리포트로 자동 이동) |
| 충돌·모순 리포트 | `/report` | 분석 결과(충돌/모순) 리포트 |
| 회차 업로드 | `/episode-upload` | 회차 원고 업로드 플로우 |
| **설정 후보 검토** | `/setting-review` | AI가 원고·설정집에서 **뽑아낸 설정 후보**를 작가가 확인·확정 |
| **회차 검사 결과** | `/episode-validation-report` | 새로 올린 회차가 **기존 설정과 충돌·모순**되는지 검사한 결과 |
...
> 딥링크: 로그인/회원가입에서 `?terms=terms|privacy`로 약관·개인정보 모달을 바로 열 수 있습니다.
...
> 리포트는 **단일 회차 검수 / 발행 전 전체 검수**(딥링크 `?mode=prePublish`) 두 모드가 있습니다.
...
> 딥링크: 사이드바 `?nav=settingDB|reports|graph|manuscripts`, 설정DB 탭 `?tab=characters|relations|timeline|worldrules|search`, 캐릭터 상세 `?modal=char-detail&charId=<id>`, 관계도 샘플 `?relGraph=triangle|prosecution|court`, 그래프 노드 `?nav=graph&node=<id>`.
...
> 딥링크: 설정 후보 검토 `?candidate=<id>`, 회차 검사 결과 `?issue=<id>`, 리포트 발행 전 검수 `?mode=prePublish`.
