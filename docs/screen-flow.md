...
Pencil `gh180EpisodeAutoPolicy` 보드는 업로드 방식·미확정 경고·자동 반영·복구 흐름을 정리합니다. 실제 화면 검증 캡처는 [데스크톱](screens/gh180-upload-policy-desktop.png)과 [320px 모바일](screens/gh180-upload-policy-320.png)입니다.

`/episode-upload`는 선행 업로드 조건 없이 두 다회차 방식을 제공합니다. 미확정 후보 안내와 안내 조회 실패는 업로드를 막지 않습니다. 전체 원고는 공백 포함 250,000자까지이며 감지·최종 저장에서 확인합니다. 단일 회차는 자동 반영이 기본이고 모든 설정 직접 검토를 선택할 수 있습니다. 다회차는 선택지 없이 순차 분석·회차별 자동 저장을 사용합니다.

자동 반영 안내는 업로드 화면의 Theme V2 토큰을 사용한 보조 정보 박스로 표시한다(2026-09-09). 내부 여백 16px, 왼쪽 안내 아이콘 16px, 아이콘과 본문 사이 10px, 문단 사이 6px, 박스 아래 12px를 유지한다. 제목과 본문은 모두 13px·줄 간격 1.65이며, 제목은 700 굵기와 `--ch-ink`, 본문은 `--ch-text-muted`, 배경과 테두리는 기존 `--ch-surface-soft`·`--ch-border`를 사용한다. 문구는 다음과 같다.

- **회차 순서대로 분석하고 설정을 자동 반영합니다**
- 앞 회차의 설정을 저장한 뒤 다음 회차를 분석합니다. 아직 확정하지 않은 제안도 참고합니다.
- 인물이나 내용이 불분명하거나 비교에 실패한 설정은 분석 후 직접 검토할 수 있습니다.

Pencil `gh180EpisodeAutoPolicy`는 선행 조건 없는 업로드와 단일 자동 반영 기본값을 함께 반영한다.

```mermaid
flowchart LR
  upload["업로드 방식 선택 · 미확정 후보 안내"] --> single["단일 회차 · 자동 반영 기본 / 전체 직접 검토 선택"]
  upload --> multi["다회차 · 순차 분석과 자동 반영"]
  single -->|"직접 검토"| review["설정 후보 검토"]
  single -->|"자동 반영"| ordered["한 회차 분석 → 명확한 설정 저장 → 다음 회차"]
  multi --> ordered
  ordered --> stopped["앞 회차 실패·저장 미완료 · 뒤 회차 재개 대기"]
  stopped -->|"중단된 회차부터 재개"| ordered
  ordered -->|"모든 회차 저장 완료"| done["분석 완료 · N개 확인 가능"]
  done -->|"원할 때 결과 보기"| pending["확인이 필요한 설정 검토 · 대기 후보 기본"]
  ordered -->|"원문·검토 변경"| invalid["새 분석 필요"]
  invalid --> manuscripts["원고 목록에서 확인"]
  invalid -->|"범위·사용량 확인 후 명시적 시작"| ordered
```

재개 응답은 같은 Job ID일 수 있으므로 캐시를 갱신하고 재조회합니다. 무효화된 실행은 자동 재분석하지 않습니다. 기존 일반 분석의 대기·재시도 동작은 유지합니다. 상세 계약은 [업로드 요구사항](data-requirements/upload.md#gh180-다회차-분석-방식)에 있습니다.

2026-09-13 리뷰 보완: 재시도로 교체된 이전 작업의 반복 조회를 멈춥니다. 새 분석은 현재 모든 회차의 상태를 확인한 뒤 허용하며 확인창 제출에도 같은 조건을 적용합니다. 원문 교체 후 단일 재분석은 직접 검토로 요청하여 확정 설정을 자동 변경하지 않습니다. 캐릭터 연결을 변경하면 해당 후보의 기존 직접 검토 완료 표시를 해제하여 새 대상의 비교·확인을 건너뛰지 않습니다. 연결 저장 실패 시에는 기존 검토와 입력을 유지합니다.

2026-09-11: 분석 목록도 앞 회차의 중단에 막힌 후행 대기를 진행 중으로 세지 않습니다. 변경 기록과 자동 저장까지 완료된 회차만 완료 수에 포함하며, 남은 설정 확인은 회차 분석 완료와 분리합니다. 자동 완료 후 캐릭터 검토와 ordered 세계관 수정·연결 모달/그룹 확정 영역에는 앞 회차 변경이 진행 중인 뒤 회차를 중단할 수 있다는 안내를 먼저 표시합니다. 안내만 추가한 상태로, **분석 중 수정 예약·분석 종료 후 자동 반영 기능은 미구현**입니다. 추가 확인 클릭이나 자동 재분석은 없습니다.

새 분석은 원래 실행의 뒤 회차만 선택하는 기능이 아니라 원본 업로드의 현재 보관 중인 전체 회차를 대상으로 합니다. 진행 중인 작업이나 이미 확정한 후반 이력 때문에 서버가 거절할 수 있으며, 오류 안내를 유지하고 기존 모드로 자동 전환하지 않습니다.
...
> **값 검증 분기**: 캐릭터 row의 `valueValidation=INVALID`면 경고를 표시하고 그룹 확정과 비교 시작·재시도를 잠근다. `repairable=true`는 수정·제외, schema 해석 오류인 `false`는 제외만 허용한다. NUMBER/BOOLEAN 수정값은 모달에서 먼저 검증하고, 저장 후 서버가 내려준 새 `valueValidation=VALID`여야 비교·확정 동선으로 돌아간다.
...
> **GH-194 분류 카드**: 승인된 판타지 이미지 8장을 카드 위에, 제목·설명을 아래에 표시한다. `전체 보기`는 넓은 세계를 조망하는 A안을 사용한다. 데스크톱은 네 열, 1050px 이하에서는 두 열로 정리하며 320px에서도 두 열과 3:2 이미지 비율을 유지한다. 분류 선택 뒤에는 이름·설정 개수 중심의 대상 카드 목록을 표시하고 클릭할 때 상세 모달을 연다. 목록의 `분류 선택으로` 버튼으로 분류를 다시 고르며 검색·정렬은 유지한다. `/demo`도 자동 상세 선택을 제거하고 같은 목록·모달을 사용한다.

## Pencil Workflow Boards

### 모바일 조작 방식 (2026-09-06)

- 768px 이하에서 대시보드의 다섯 설정 탭은 `작품 설정` 선택 메뉴로 바뀐다. 선택 시 기존 `tab` 전환 함수를 사용하며, 세계관 진입은 분류 선택 화면부터 시작한다. 세계관 목록의 `분류 선택으로` 버튼으로 이미지 분류 선택 화면에 돌아간다. 목록 안의 분류 선택 메뉴는 제공하지 않는다.
...
- 캐릭터 상세는 실제 header 높이를 제외한 본문을 스크롤한다. 세계관 상세는 모달 헤더를 유지하며 본문을 스크롤하고, 직접 입력은 한 열 폼과 하단 저장·취소 영역을 사용한다. 320×568 화면에서도 마지막 입력과 저장 액션에 접근할 수 있어야 한다.
...
| --- | --- | --- |
| 랜딩 | [`/landing`](https://catch-hole.vercel.app/landing) | 로그인 없는 공개 데모를 주 CTA로 안내하는 Theme V2 페이지. 영상 Hero(데스크톱 스크롤 진행·모바일 일반 재생) → 8단계 실제 제품 데모 → 원고·분석 결과의 AI 학습 미사용 안내(`#manuscript-protection`)와 `/privacy` 링크 → 핵심 기능 3항목 요약 → 주요 서비스 카탈로그 → 마지막 시작 안내 순서로 표시한다. 모바일에서도 로그인 진입점을 유지한다. |
...
| **세계관 DB 조회** | [`/dashboard?nav=settingDB&tab=worldsettings`](https://catch-hole.vercel.app/dashboard?nav=settingDB&tab=worldsettings) | 이미지 분류 선택·대상 카드 목록·상세 모달에서 확정 설정과 원문 근거를 조회 |
...
flowchart TD
  ...
  nav_settingDB --> t_char & t_worlddb & t_rel & t_world & t_search
  ...
  m_chardetail -. "변화 이력 보기" .-> m_chartimeline["우측 캐릭터 설정 이력 패널<br/>현재 설정 즉시 복수 선택 · 회차 바로가기 · cursor 조회"]:::modal
  m_chartimeline -. "근거" .-> m_timeline_evidence["같은 우측 영역의 CharacterFact 원문 화면<br/>전체 원문 · quote 하이라이트"]:::modal

  subgraph WORLD_SETTINGS["확정 세계관 관리"]
    direction TB
    world_categories["이미지 분류 선택"]
    world_list["대상 카드 목록<br/>검색·정렬·페이지"]
    world_detail["선택 대상 상세 모달<br/>범위별 설정·원문 근거"]:::modal
    world_create["새 대상 추가 모달<br/>분류·대상·첫 설정"]:::modal
    world_edit["대상 정보 수정 모달"]:::modal
    world_property["설정 추가·수정<br/>상세 인라인 편집"]:::modal
  end

  t_worlddb --> world_categories
  world_categories -- "분류 선택" --> world_list
  world_list -- "분류 선택으로" --> world_categories
  world_list -- "대상 선택" --> world_detail
  world_detail -- "닫기 · 목록 위치 유지" --> world_list
...
> 세계관 DB와 설정 검색은 각각의 `q`·`page`를 보조 URL 키에 저장했다가 활성 탭의 공통 키로 복원합니다. 세계관 DB는 뒤로가기·분류명·개수·추가 버튼을 한 줄로 압축하고 모든 크기에서 목록을 먼저 표시하며, 900px 이하에서는 검색·정렬을 1열로 쌓습니다. 실제 목록 여유 높이와 열 수에 따라 12·18·24개로 페이징하며 `worldSize`에 경계를 저장합니다. 새로고침·공유·뒤로가기는 저장한 경계를 복원하고, 열 수/수용량이 바뀌면 기존 첫 항목을 포함하는 페이지로 보정합니다. 분류 선택 복귀와 탭 전환은 크기를 초기화합니다. 대상 클릭은 `settingId`가 있는 상세 모달을 열고, 닫기는 검색·분류·정렬·페이지·목록 스크롤을 유지합니다. 상세 공유 링크는 목록 조회 결과와 독립적으로 열립니다.
...
> 캐릭터 DB와 세계관 DB의 조회 플로우는 같은 Theme V2 헤더·탭·surface·상태 패턴을 사용합니다. 캐릭터는 카드 → 상세 → 우측 변화 이력 → 원문 근거 순서, 세계관은 분류 선택 → 대상 카드 목록 → 상세 모달 → 펼친 원문 근거 순서를 사용하며, 테마 전환은 URL·API·캐시 경계를 변경하지 않습니다.
...
> 사이드바의 남은 사용량 게이지·퍼센트 표시는 제거합니다. 분석 생성·재시도에서 `AI_TOKEN_QUOTA_EXHAUSTED` 응답을 받으면 요청을 시작한 확인 모달을 닫고 전역 안내 모달을 연 뒤 미처리 추가 사용량 요청부터 조회합니다. 미처리 요청이 없을 때만 35~1000자 피드백 입력과 `제출`·`취소`를 제공하며, 제출한 뒤에는 운영자 검토 중 상태로 전환합니다. 운영자는 별도 Backend API에서 수동 승인·반려하고, 승인 시 신규 회원 초기 지급량과 같은 설정값을 추가합니다. 누적 지급량과 사용량은 초기화하지 않습니다.

> 실제 업로드한 회차가 활성 작품 합계 3개 이상이고 일반 의견을 아직 보내지 않은 계정에는 원고 목록에서 한 번 의견 안내를 띄웁니다. 서버의 노출 권한 부여 API가 계정별 기록을 저장하고 `shouldShow=true`를 반환한 경우에만 표시합니다. 진행 중 분석·다른 모달·입력 중에는 보류하며, `의견 남기기`는 기존 작성 창으로 연결하고 `닫기` 후에는 다시 자동 표시하지 않습니다. 노출 서버 오류 시에는 수동 작성 기능을 유지합니다. (#196)

> 인증 보호 화면의 사용자 아바타 왼쪽에는 `의견 보내기`를 항상 표시합니다. 상시 의견은 사용량 소진 여부와 무관하게 매번 저장하며, 첫 일반 피드백에서만 회원당 한 번인 `GENERAL_FEEDBACK_REWARD` 추가 사용량 요청을 함께 만듭니다. 다른 추가 사용량 요청이 처리 대기 중이면 의견은 저장하고 보상 요청만 보류합니다. 처리 뒤의 추가 의견도 계속 저장하지만 이미 일반 피드백 보상 요청 이력이 있으면 새 요청을 만들지 않습니다.

```mermaid
flowchart TD
  trigger["상단 의견 보내기"] --> form["10~1000자 의견 입력"]
  form -->|제출| save["의견 새 행 저장"]
  save --> lifetime{"일반 피드백 보상 요청 이력?"}
  lifetime -->|있음| feedback_only["의견 접수 안내"]
  lifetime -->|없음| pending_other{"다른 요청 PENDING?"}
  pending_other -->|있음| deferred["의견 저장 · 보상 요청 보류"]
  pending_other -->|없음| reward["GENERAL_FEEDBACK_REWARD 요청 생성"]
  reward --> reviewing["운영자 검토"]
```

```mermaid
flowchart TD
  exhausted["사용량 소진 이벤트"] --> pending["미처리 요청 조회"]
  pending -->|있음| reviewing["운영자 검토 중 안내"]
  pending -->|없음| feedback["피드백 입력"]
  feedback -->|35자 미만 또는 1000자 초과| feedback
  feedback -->|유효한 제출| creating["요청 저장 중"]
  creating -->|성공| reviewing
  creating -->|실패| retry["입력 유지 · 다시 시도"]
  retry --> feedback
```

> 피드백에는 원고 본문이나 민감정보를 자동 첨부하지 않습니다. 서비스 이메일은 보조 연락처로 계속 표시하고, 밝은 모달 surface에서 의미 색상·밑줄·4.5:1 이상의 실제 렌더링 명암비를 유지합니다. 요청 body의 `context`는 동기 차단, 1차 분석 실패, 1차 추출 뒤 비교 중단을 구분합니다.

> 분석 시작 뒤 비동기 Job에서 같은 failure code가 확인돼도 전역 안내 모달을 엽니다. 같은 배치의 현재 Job이 모두 종료될 때까지 최종 안내를 미루고, 1차 추출 전 실패는 대상 수에 따라 `회차 분석`, `전체 회차 분석`, `일부 회차 분석` 중단으로 구분해 실패 회차 재시도를 안내합니다. 1차 추출 뒤 세계관 비교가 중단된 경우에는 `51개 세계관 설정 비교가 사용량 부족으로 중단됐습니다.`처럼 건수를 표시하고, 완료된 추출·비교가 보존됐음을 안내합니다. 두 실패가 섞이면 한 모달에 실패 회차와 보존된 비교 수를 함께 표시합니다. Pencil 기준 프레임은 `qtP124`(`SEpisodeUpload / 5. 세계관 비교 일부 중단`)입니다.
...
> 원고 목록 행에는 분석 `진행 보기`·`결과 보기`·`다시 시도`와 `미처리` 열을 두지 않습니다. 진행·부분 실패·실패·설정 후보 검토 필요 상태는 목록 위 배너에서 알리고, 실제 후속 액션은 업로드 묶음별 분석 목록에서 제공합니다. 원문 변경으로 `재분석 필요`가 된 회차의 `재분석` 액션은 유지하되, 해당 회차만 다시 분석하고 후속 회차에서 축적된 설정 때문에 후보의 시간 순서가 섞일 수 있다는 확인 모달을 먼저 표시합니다. 재분석 시작이 사용량 부족으로 거절되면 이 확인 모달은 즉시 닫고, 사용량 안내를 닫은 뒤 다시 노출하지 않습니다. 분석 중인 회차의 파일 변경·삭제·중복 분석 요청은 비활성화합니다.

> 배치 조회가 분석 시작을 확인하면 원고 목록도 즉시 갱신하며, 진행 중에는 10초 갱신을 함께 유지합니다. 배치 종료 시 최종 목록을 한 번 더 받고 진행 작업이 없으면 반복 조회를 멈춥니다. 업로드·재분석 시작 성공 때에도 두 조회의 캐시를 갱신해 이전 `재분석 필요` 표시가 남지 않게 합니다.
...
> - 설정DB 탭 — [캐릭터 DB](https://catch-hole.vercel.app/dashboard?nav=settingDB&tab=characters) · [세계관 DB](https://catch-hole.vercel.app/dashboard?nav=settingDB&tab=worldsettings) · [관계도](https://catch-hole.vercel.app/dashboard?nav=settingDB&tab=relations) · [설정집 목록](https://catch-hole.vercel.app/dashboard?nav=settingDB&tab=worldrules) · [검색](https://catch-hole.vercel.app/dashboard?nav=settingDB&tab=search). 캐릭터의 타임라인은 캐릭터 상세 `변화 이력`에서 연다.
...
업로드 방식 선택 후 계정 전체의 분석 이력이 0건이고 아직 안내하지 않은 작가에게 설정 반영 방식 안내를 한 번 연다. `반영 방식 → 모든 설정 직접 검토 → AI 자동 반영 → 자동 반영 후 직접 검토 → 검토 완료`를 한 줄 단계 바와 방향키/이전·다음으로 이동한다. 최대 1,320px 모달에서 실제 업로드·검토·프로필 컴포넌트를 로컬 예시로 공유한다. PNG와 방식 탭은 없고 예시 이벤트는 차단한다. 전체 3개 검토, 명확한 2개 자동 저장, 모호한 1개 직접 확인의 차이를 보여준다. 다회차에는 직접 검토 선택이 단일 회차용임을 명시한다. 닫아도 업로드 입력·반영 방식은 유지하며 도움말로 언제든 다시 볼 수 있다. 조회/선점 실패는 업로드를 막지 않는다. [상세 노출·URL 계약](analysis-mode-guide.md). Pencil: `gh194AnalysisGuideDesktop`, `gh194AnalysisGuideMobile`.
...
업로드 방식 카드는 Tab으로 이동하고 Enter·Space로 선택할 수 있는 버튼이며, 선택 상태를 `aria-pressed`로 전달합니다. 회차 제목 편집 입력은 `N화 제목`, 세계관 검색 입력은 `세계관 설정 검색`이라는 접근성 이름을 유지합니다.
...
> 토큰 부분 중단은 예외적으로 Job이 `FAILED`여도 대상 회차와 1차 추출 결과를 완료 상태로 보존합니다. 업로드 진행·분석 목록은 `tokenInterruptedAfterExtraction`과 배치의 중단 건수로 이 상태를 판별하며 일반 실패 회차 재시도 목록에서 제외합니다. 현재 대상 회차가 `ARCHIVED`이면 이 예외보다 이용 불가 상태를 우선해 보존 결과 검토로 이동시키지 않습니다.
...
범위가 없는 후보가 기존 scoped 설정과 겹치면 자동 적용하지 않고 `범위 확인 필요`로 남깁니다. 모든 source가
동일한 기존 경로를 가리키고 필터로 숨은 source가 없을 때 `기존 범위 › 설정명에 병합`을 누르면 수정 모달에
그 경로와 `MERGE`가 채워지며, 사용자가 최종값을 확인한 뒤 연결된 source 전체를 한 번에 저장합니다.

세계관 batch 예상 출력이 16,000 token을 넘은 후보는 `출력 한도 검토`로 구분하고 자동 판단·확정을 하지
않습니다. 수정 모달에는 1차 원문의 범위·설정명·값과 기본 `ADD`를 채우며, 사용자가 반영 방식과 최종값을
저장한 뒤에만 그룹 확정을 허용합니다. 서로 다른 원문값은 하나의 최종값으로 확인하기 전까지 충돌 상태입니다.
미리 채운 충돌값을 그대로 두거나 공백·줄바꿈만 바꿔 반영하려 하면 수정 모달에서 오류를 표시하고 입력을
유지합니다. 최종값을 정리하기 전에는 저장 요청을 보내지 않으며, 반영하지 않는 제외는 허용합니다.
...
> `FAILED + AI_TOKEN_QUOTA_EXHAUSTED` 후보는 단건 재비교 대신 상단 배치 배너에서 정확한 중단 건수와 `남은 비교 재개`를 제공합니다. 버튼은 토큰 중단 후보만 재사용하고 완료·확정·제외·다른 실패 후보를 건드리지 않으며, 요청 뒤 기존 polling으로 `PENDING → PROCESSING → COMPLETED/FAILED`를 갱신합니다. 한 그룹에 일반 실패가 함께 있으면 `비교 중단·실패 혼합`으로 표시하고 배치 재개와 일반 `다시 비교`를 모두 안내합니다. 목록의 활성 비교 Job 수가 0이 될 때까지 재개된 `PENDING`의 단건 재시도와 최종 중단 모달을 막아 탭 전환·새로고침에서도 배치 재개 흐름을 유지합니다. 활성 Job이 끝나도 실패·재비교 필요 후보가 남으면 성공이 아니라 확인 필요로 표시합니다. Pencil 기준 프레임은 `wrP124`(`SSettingReview / 세계관 비교 일부 중단 · 일괄 재개`)입니다.

> 세계관 후보 목록의 최초 조회가 실패해도 공통 캐릭터·세계관 후보 탭은 오류 상태 위에 유지하여 정상 조회 가능한 다른 후보 종류로 이동할 수 있습니다.

> 범위가 다른 `SCOPE_MISMATCH` 후보는 `범위 비교 필요` 상태에서 원문 범위·항목과 비교한 기존 범위·항목을 함께 확인합니다. 수정창에서도 두 경로를 안내하고, 반영할 경로와 작업을 직접 저장한 뒤에만 그룹 확정을 허용합니다. 확인 필요 안내는 밝은 바탕·주황색 강조선, 반영안 저장 완료는 연한 파란색으로 구분하며 저장과 작품 설정 반영을 구별합니다. Pencil 기준 프레임은 `gh180ScopeReview20260910`입니다.

> 세계관 후보의 대상 연결 보류는 `대상 연결 확인 필요`, 범위 미정은 `범위 확인 필요`, 입력량 초과는 `비교 분량 확인 필요`로 구별합니다. 서버의 보류·실패 안내를 AI 판단으로 소개하지 않으며, 대상이 미정이면 기존값도 `비교 대상 미정`으로 알립니다. 범위가 없고 기존 항목명과 달라도 원문과 비교 경로를 모두 유지하고 직접 저장한 뒤 확정합니다. 진단이 제공된 실패 후보는 마지막 비교에서 선택한 범위·항목명만 확인할 수 있고, 선택 기록이 확정된 연결은 아니라는 설명을 함께 제공합니다. Pencil 기준 프레임은 `gh180ReviewReasons20260910`입니다.

> 세계관 검토 그룹은 같은 분류·이름으로 모인 후보입니다. 그룹으로 보인다는 사실만으로 같은 주체라고 소개하지 않습니다. 미수정·비교 완료 후보의 응답에 서로 다른 기존 대상 연결이 명시된 경우만 연결 확인을 안내하며, 대상 ID가 없는 신규 후보의 보류 원인은 추정하지 않습니다. 상세 보류 사유 API는 후속 검토 사항으로 남기고 현재 수정·확정 흐름은 유지합니다. Pencil `gh180ReviewReasons20260910`의 묶음 기준 설명과 함께 관리합니다.

> 검토 요약은 반영됨·제외됨·직접 확인을 중심으로 보여 주고 분석 중은 별도 상태로 분리합니다. 자동 반영 보류·수정 저장 후 미확정·발견 후보도 직접 확인에 남으며 후보마다 한 번만 셉니다. 탭·필터·페이지와 관계없이 같은 전체 집계를 사용합니다. 상세 사유는 연결·범위·비교 실패·같은 대상 확인 등의 배지로 구분합니다. 원문·고유명사·설정값은 보존하고 안내는 자연어로 설명합니다. Pencil `gh180DirectReview20260910Desktop`, `gh180DirectReview20260910Mobile`을 기준으로 관리합니다.

> 자동 회차의 개별 비교가 끝나도 회차 자동 반영이 끝날 때까지 해당 후보는 분석 중입니다. 현재 회차의 수정·제외·연결·확정은 잠기고 이전 완료 회차 보류는 개별 검토할 수 있습니다. 열린 수정 창도 입력을 보존한 채 저장만 잠급니다. 필터·페이지에 숨은 분석 후보도 완료까지 갱신하며 완료 후 반복 조회를 멈춥니다. 검증 화면: [캐릭터 320px](screens/gh180-auto-pending-character-320.png), [세계관 320px](screens/gh180-auto-pending-world-320.png), [캐릭터 데스크톱](screens/gh180-auto-pending-character-1280.png), [세계관 데스크톱](screens/gh180-auto-pending-world-1280.png).

> 목록 복귀 버튼은 밝은 보조 버튼으로 표시합니다. GH-194 세계관 상세는 모달의 닫기 버튼으로 목록에 복귀합니다. 흰색 표면·기본 테두리·파란색 안내 글자 토큰을 사용하고 최소 높이 44px, 4.5:1 이상의 명암비, 키보드 초점 표시를 유지합니다.
...
> 2026-09-11: 직접 해결 가능한 후보 비교 미완료는 `검토 필요`와 `자동 비교를 마치지 못해 대상과 내용을 확인해 주세요.`로 표시한다. 실패 기록은 유지하며 직접 저장 후 확정한다. 수정 불가능한 값은 잠긴 버튼을 누르라고 안내하지 않고 원문 확인·제외를 안내한다. 실제 회차 중단·후속 대기·사용량 부족과 재시도 동작은 바뀌지 않는다. 정상 일반 보류는 `대상·내용 확인 필요`, 해당 반영 방식 필터는 `AI 판단 보류`로 구분한다. 전체 반영·제외·직접 확인·분석 중 집계는 서버 기준을 유지한다.

## 랜딩 영상 Hero (NVM-321)

`/landing`의 기존 첫 화면을 교체한다. 새 라우트를 만들지 않는다.

- 설정 추출 장면의 안내는 `작성한 원고를 업로드하면 / 캐릭터와 세계관 설정을 추출해요.`로 표시한다.
- 데스크톱: 여백·둥근 모서리가 있는 작가 장면 → 스크롤/드래그로 확장·영상 진행 → 노트북 마지막 프레임 → 25화 원고 → 원고 주변의 설정 6개 → 종류별 확정 → 체험·무료 시작 CTA → 기존 제품 데모와 하단 소개. 같은 스크롤 위치에서 같은 장면을 표시한다.
- 모바일(680px 이하): 첫 화면 영상 일반 재생 → 같은 위치에서 25화 원고 표시 → 세계관·캐릭터·마법 설정 6개가 세로로 순차 추출 → 종류별 확정 → 완료 문구와 체험·무료 시작 CTA → 기존 제품 소개. 영상 이후 약 18초의 시간 기반 진행이며 스크롤 위치와 연결하거나 자동 스크롤하지 않는다. 영상과 추출의 일시정지·재개, 직접 확정, 전체 다시 보기를 제공한다. 모달·탭 비활성 시 진행을 멈춘다.
- 모션 감소: 자동 재생·스크롤 애니메이션 없이 수동 재생과 정적 장면 탐색을 제공한다. 모바일 영상이 끝난 뒤 추출 재생 버튼은 숨기고 종류별 수동 확정과 완료 후 다시 보기만 제공한다.
- 백그라운드 탭에서 처음 열면 영상 시작을 보류하고 탭이 보일 때 재생한다. 이미 재생 중이던 탭을 숨기면 멈추며, 복귀 후에는 사용자가 직접 재개한다.
- 높이가 낮은 데스크톱에서도 설정 본문·상태 글자의 최소 표시 크기를 유지하고, 오른쪽 두 카드의 간격은 실제 카드 높이를 기준으로 확보한다.
- 영상 탐색·설정 확정은 소개용 로컬 상태이며 API·로그인·설정 DB를 변경하지 않는다. CTA는 기존 `/demo`, `/login`, `/signup` 동선을 유지한다.

> 최신 main 통합: 영상 Hero의 장면·CTA 시점은 유지하며 서비스 신뢰 안내만 `필요한 내용만 직접 확인`으로 맞춘다. 기존 범위 병합 바로가기는 같은 판단의 모든 후보가 자동 저장 대기를 벗어난 뒤에만 사용한다.

## GH194 공용 대표 이미지 선택

`이미지 변경 → 내 이미지`에서 보관함 생성/복구키 보관 → 암호화 업로드 → 선택/저장으로 이어진다. 재방문 시 복구키로 잠금을 풀며, 업로드·삭제 중에는 창 닫기와 탭 전환을 막는다. 사용 중인 이미지 삭제는 거절하고 대표 이미지 변경을 안내한다. [개인 이미지 흐름과 보호 범위](private-world-images.md).

분류 선택 → 대상 카드 목록(썸네일·이름·설정 개수) → 상세 모달 → 이미지 변경 → 같은 분류 장르 추천(전체 도감 전환 가능) → 검색/선택 → 이미지 저장 → 상세 모달. 기본 이미지 선택으로 연결을 해제할 수 있다. 이미지 저장은 설정 내용·설정 version·분석 상태를 바꾸지 않는다.

`settingId`를 유지한 채 `modal=world-setting-image`를 열고, 이미지 창을 닫으면 modal만 제거해 상세로 돌아간다. 상세를 닫으면 기존 목록의 검색·정렬·페이지·스크롤·초점이 유지된다. 충돌은 초안을 보존하고 최신 조회 후 사용자가 다시 저장한다. `/demo`에는 API 호출·실제 이미지 변경을 추가하지 않는다.

### GH194 장르 이미지 연결

작품 장르 → 테마별 분류 8칸 → 대상 목록/상세의 기본 그림 7종 → 이미지 변경의 `장르 추천`으로 이어진다. `전체 도감`은 다른 장르의 공용 사진까지 같은 분류 안에서 선택하게 한다. 작품 장르 수정은 미선택 기본 그림과 추천을 갱신하고 수동·개인 선택은 유지한다. [구현·검증](world-image-themes.md).

### GH194 저장된 자동 이미지 선택 (2026-09-17)

캐릭터·세계관의 자동 사진은 서버가 확정/수정 시 연결해 저장한다. 세계관 이미지 모달에서도 대상 이름에 따른 자동 선택 복귀와 분류 기본 이미지 고정을 구분한다. 수동 도감·개인 이미지는 기존과 동일하고 409에는 초안을 유지한다. 조회마다 추가 매칭/LLM 호출은 없다.
