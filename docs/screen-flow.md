...
업로드 방식 선택 후 계정 전체의 분석 이력이 0건이고 아직 안내하지 않은 작가에게 설정 반영 방식 안내를 한 번 연다. `원고 → 분석 결과 → 직접 확인 → 저장 결과`를 방향키/이전·다음으로 이동하고 예시 확정 버튼을 눌러 본다. 단일 회차는 자동 반영과 직접 검토를 비교하고, 다회차는 자동 반영만 설명한다. 닫아도 업로드 입력·반영 방식은 유지하며 도움말로 언제든 다시 볼 수 있다. 조회/선점 실패는 업로드를 막지 않는다. [상세 노출·URL 계약](analysis-mode-guide.md). Pencil: `gh194AnalysisGuideDesktop`, `gh194AnalysisGuideMobile`.
