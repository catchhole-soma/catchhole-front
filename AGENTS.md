...
- 후보의 `valueValidation.status=INVALID`는 row 내 danger 경고와 Backend `message`로 표시하고, 수정·제외은 허용하되 그룹 확정은 잠그다. NUMBER 수정값은 숫자 문자열, BOOLEAN은 소문자 `true`/`false`로 프론트에서도 검증하고, 저장 성공 후 재조회한 `valueValidation`으로 잠금 해제를 판단한다. `valueJson`을 클라이언트에서 수정·추측하지 않는 기존 경계를 유지하면서, 사용자가 본 값과 실제 저장값의 불일치를 방지하기 위함이다.
