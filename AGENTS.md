...
- 세계관 후보 목록·상세는 같은 `batchId`의 `분류 + 대상` 그룹과 key diff row로 표시하되 후보 ID·비교 상태는 row별로 유지한다. 그룹 확정·제외는 전용 단일 요청을 사용하고 기존 단일 후보 mutation을 반복 호출하지 않는다.
...
- 같은 설정명의 여러 1차 추출값은 AI가 후보 하나로 통합하고 `SINGLE/MERGED/CONFLICT` 상태를 반환한다. Front는 `MERGED`를 `여러 내용 정리됨`, `CONFLICT`를 `내용 확인 필요`로 표현하고 내부 enum을 노출하지 않는다. `CONFLICT` row는 기본 미선택이며 사용자가 수정에서 최종값을 저장하기 전에는 반영 확정을 잠그되 선택 제외는 허용한다. 모든 `evidenceSpans` quote는 생략 없이 표시한다.
...
- 확정 세계관 목록은 `분류 + 대상`을 한 항목으로 표시하고 문자열 key/value 설정 객체를 상세에서 펼친다. FE가 설정별 평면 행으로 재구성해 대상 중복을 만들거나 JSON 전체를 덮어쓰지 않는다.
