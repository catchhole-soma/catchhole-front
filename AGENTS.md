...
## 세계관 설정 (NVM-260 구현 계획)

- NVM-260 구현 시 `/setting-review`는 캐릭터·세계관 후보의 공통 검토 화면으로 유지하되 두 후보를 한 목록이나 한 DTO로 합치지 않고 탭과 데이터 계약을 분리한다. 현재 구현은 캐릭터 후보만 지원하며, 캐릭터 후보의 기존 저장·확정 규칙을 세계관 후보 구현 때문에 변경하지 않는다.
...
- 세계관 DB의 직접 입력은 새 대상·설정 추가와 수정까지만 제공한다. 삭제·보관·복원과 구체적인 API 경로·DTO는 계약이 확정되기 전 구현하지 않는다.
