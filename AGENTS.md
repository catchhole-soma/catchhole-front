...
- 최초 분석 방식 안내는 계정 전체 기준이다. `analysis-mode-guides` 조회/선점 API의 결과만 자동 노출에 사용하고 브라우저 저장소나 현재 작품의 개수로 대체하지 않는다. 한 번 선점한 뒤에는 수동 도움말만 제공한다. 안내 내부는 실제 업로드·검토·프로필 컴포넌트에 로컬 예시 데이터를 넣는 읽기 전용 화면이다. 캡처 이미지를 쓰지 않으며 반영 방식 → 전체 직접 검토 → AI 자동 반영 → 자동 반영 후 직접 검토 → 검토 완료의 한 줄 5단계만 제공한다. 대상 선택·확정 실습이나 실제 분석·설정 변경 요청을 넣지 않고 기존 업로드 선택을 유지한다. 상세 계약은 `docs/analysis-mode-guide.md`를 따른다.
...
- 교체된 과거 Job은 반복 조회하지 않습니다. 현재 ordered Job은 완료 뒤 무효화 감지를 위한 조회를 유지합니다. 새 순차 분석은 현재 Job 전체의 응답이 확인되고 실행 중인 회차가 없을 때만 확인창과 제출을 허용합니다.
...
- GH201은 기존 회차별 Job 재개와 전체 완료 뒤 batch 검토를 유지한다. 부분 검토·잔여 취소·부분 종료를 추가하지 않는다. `ANALYSIS_FUTURE_HISTORY_CONFLICT`로 생성이 거절되면 같은 batch의 자동 생성 버튼을 반복 제공하지 않고 저장된 원고 목록의 기존 개별 직접 검토 경로를 안내한다. `ANALYSIS_ORDERED_JOB_RETRY_REQUIRED`는 기존 분석 목록으로 안내한다. 일반 모드 오류·일시적 작업 충돌을 영구 거절로 추측하지 않는다. 재개 응답 유실/충돌 뒤에는 기존 Job을 다시 조회하며 새 분석을 만들지 않는다. 자세한 계약은 `docs/analysis-recovery.md`를 따른다.
- 세계관 순차 후보의 복구 안내는 `sourceAnalysisJobStatus`·`sourceAnalysisJournalStatus`를 사용해 진행·실패·무효화를 구분한다. 후보 PENDING만으로 중단을 단정하지 않고 원본 상태 없는 구응답은 확인 안내로 둔다. 사용량 부족은 일반 후보만 서버의 배치 전체 `canResumeTokenInterruptedComparisons`에 따라 비교 재개를 제공하고, 순차 후보는 기존 회차 분석 목록으로 안내한다. 별도 비교 Job 수와 현재 페이지의 후보로 원본 실행 상태·전체 배치 재개 가능 여부를 추측하지 않는다.
- ordered 후보는 개별 재비교 API를 호출하지 않는다. 자동 완료 후 서버가 직접 검토를 허용한 후보는 기존 검토 흐름을 유지하고, legacy/중단 후보에 개별 재비교 버튼이나 자동 재비교 effect를 제공하지 않는다.
...
- 파일을 교체한 회차의 `재분석`은 원문 청킹과 캐릭터·세계관 후보 재추출이 목적이므로 `episodeId`를 지정한 `SETTING_EXTRACTION` Job을 생성합니다. 시작 전에는 변경한 회차만 다시 분석하고 이미 완료된 뒤 회차의 결과를 보존한다고 안내합니다. 확정 설정은 자동 변경되지 않으며 직접 검토·확정할 때 뒤 회차의 같은 항목과 직접 수정한 값을 보호하면 이력에 저장하고, 그 외에는 선택한 반영 방식에 따라 현재 설정에도 반영할 수 있습니다. 구현되지 않은 충돌 검수용 `EPISODE_VALIDATION`으로 보내지 않되 과거 검수 Job의 진행 화면 조회 호환은 유지합니다.
- 원문 교체 후 재분석은 `reviewMode=MANUAL`을 명시해 기존 확정 설정을 자동 변경하지 않는 안내를 지킵니다. 새 원고 업로드의 기본 자동 반영 정책은 유지합니다.
- GH201 원문 교체는 완료된 뒤 회차의 분석을 자동 재시작하지 않는다. 진행·실패로 입력이 고정된 미완료 분석의 변경 제한은 서버 판단을 표시한다. 확정 결과의 `historyOnly`는 `CONFIRMED_ONLY`에서도 서버 응답대로 표시하며 프론트가 회차 번호·mode·제안 operation만으로 현재값 반영을 추측하지 않는다.

- 일반 의견은 trim 후 Unicode 10~1000자이며 별도 사용량 소진 요청의 35~1000자 제약과 구분한다. 자동 의견 안내는 실제 업로드 회차가 활성 작품 합계 3개 이상인 계정 중 기존 의견·노출 이력이 없는 경우, 원고 목록의 안전한 시점에만 서버의 일회 노출 권한을 받은 뒤 표시한다. 브라우저 저장소나 마지막 회차 번호로 계정별 대상 여부를 추정하지 않는다.
...
- 캐릭터 단건·일괄 연결 저장이 성공하면 요청에 포함된 후보의 로컬 직접 검토 완료 표시를 해제한다. 이전 대상에 저장한 확인으로 새 대상의 비교·검토를 건너뛰지 않으며, 연결 실패와 다른 후보의 검토 표시는 유지한다.
...
- `CONFIRMED_ONLY` 캐릭터 설정 후보는 해당 그룹의 최신 `COMPLETED` 비교 revision이 있어야 확정한다. 서버가 명시한 `analysisMode=ORDERED_PROVISIONAL` 후보는 회차별 비교 문맥을 사용하므로 batch 공통 revision을 요구하지 않고 기존 비교 완료·자동 반영 대기·직접 수정 확인 규칙을 적용한다. 신규 캐릭터는 빈 snapshot에서 같은 그룹 후보를 순서대로 비교하고, `PENDING`·`PROCESSING`·`WAITING_FOR_CHARACTER_MATCH`이나 누락·이전 revision에서는 확정을 잠근다. 배포 전 후보가 `WAITING_FOR_CHARACTER_MATCH + UNRESOLVED`, 비교 누락, 또는 `NOT_REQUIRED`로 남아 있으면 `현재 설정 비교 시작` retry를 제공한다. `FAILED`·`RECOMPARISON_REQUIRED`는 상세와 1차 원문 근거를 유지한 채 retry를 제공하며, `EXCLUDE`는 기존 무시 액션, `REVIEW_REQUIRED`는 후보 수정·재비교 또는 이력 저장으로 유도한다.
...
- GH194 세계관 DB는 이미지 분류 선택 → 이름·설정 개수 중심의 대상 카드 목록 → 상세 모달 순서다. 목록 안의 분류 전환 UI 대신 `분류 선택으로`를 제공하고 검색·정렬·페이지는 유지한다. 모든 크기에서 첫 대상을 자동 선택하지 않으며 상세 API는 선택한 대상에만 요청한다.
- 세계관 목록의 상단은 뒤로가기·분류명·조회 개수와 추가 버튼으로 압축한다. 실제 목록 여유 높이와 열 수에 따라 12·18·24개 중 페이지 크기를 선택하고 `worldSize`에 저장한다. 새로고침·공유·뒤로가기는 저장한 경계를 복원하며, 열 수/수용량 변경 시 기존 페이지 첫 항목이 포함된 페이지로 이동한다. 검색·정렬은 크기를 유지하고, 분류 선택 복귀·분류 선택·탭 전환은 초기화한다. 설정 검색의 `size=20`과 혼용하지 않는다.
- 세계관 상세 모달을 닫으면 `settingId`·`modal`만 제거하고 목록 검색·분류·정렬·페이지·스크롤·초점을 유지한다. 공유 링크의 상세는 목록 빈 상태·오류·페이지 보정과 독립적으로 표시한다. 미저장 설정 입력은 대상 ID에 묶고, 닫기 취소 확인과 중복 저장 방지를 유지한다. `/demo`는 같은 목록·모달을 fixture로 사용하되 안내문 접근과 읽는 위치를 유지한다.
...
- GH199에서는 완료된 앞 회차의 후보 변경이 후속 완료 분석을 취소하지 않으며 최신값 충돌 시 이력에만 저장하므로 자동 완료 후 캐릭터 검토와 ordered 세계관의 수정·연결 모달/그룹 확정 영역에 영향을 미리 안내한다. 이 안내는 기존 mutation을 바꾸거나 추가 확인 클릭을 요구하지 않는다. 분석 중 수정 예약·종료 후 자동 반영은 미구현이며, 예약된 것처럼 안내하지 않는다. 새 자동 보류 사유는 `SUBJECT_RESOLUTION_FAILED`를 `연결할 대상 확인`, `COMPARISON_INPUT_TOO_LARGE`를 `비교할 내용 확인`으로 표시한다.
...
- 공용 이미지 URL은 등록된 SHA의 `/api/v1/world-image-assets/*.webp`만 API base에 연결한다. 없거나 읽기 실패한 공용 이미지는 현재 테마 기본 그림을 거쳐 번들 중립 기본 이미지로 복귀한다. `/demo`는 도감 API를 호출하지 않으며 번들 이미지와 기존 읽기 전용 안내를 사용한다.
- 개인 이미지(`source: PRIVATE`)는 `docs/private-world-images.md`를 따른다. 새 업로드는 코드 없이 로그인·작품 소유권으로 접근한다. 인증 SDK로 PNG를 받고 세션별 Query와 Blob URL을 사용하며 로그아웃/계정 변경/언마운트에 이전 이미지를 제거한다. 이전 CHI1 행은 보존하고 원본 재업로드·교체를 안내한다. 새 흐름에 보관함 생성·복호화 코드를 추가하지 않는다. 원고 보호 방식 변경은 보류다.
