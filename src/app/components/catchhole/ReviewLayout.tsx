...
export function ModeCard({ icon, title, desc, color, selected, onSelect, disabled = false, badge }: {
  icon: React.ReactNode; title: string; desc: string; color: string; selected: boolean; onSelect: () => void; disabled?: boolean; badge?: string;
}) {
  ...
  return (
    <button
      ...
      className={`mode-card${selected ? ' is-selected' : ''}${disabled ? ' is-disabled' : ''}`}
      onClick={onSelect}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{
        flex: 1, minWidth: 0, borderRadius: 12, padding: '28px 18px', textAlign: 'center', cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
