...
export function FeedbackDialog({ memberId, autoPromptAllowed = false }: {
  memberId?: number;
  ...
  const invitationCloseRef = useRef<HTMLButtonElement>(null);
  ...
  const { markFeedbackSubmitted } = useFeedbackPrompt({
    memberId,
    allowed: autoPromptAllowed && !open,
    onShow: showInvitation,
  });
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          onOpenAutoFocus={event => {
            ...
            event.preventDefault();
            if (invitation) invitationCloseRef.current?.focus();
        ...
        >
          <div className="feedback-dialog__header">
            ...
            <div className="feedback-dialog__heading">
              <Dialog.Title>{invitation ? '캐치홀을 사용해 주셔서 감사합니다!' : resultCopy?.title ?? '서비스 의견 보내기'}</Dialog.Title>
              <Dialog.Description>
                {invitation
                  ? '캐치홀은 주니어 개발자들이 함께 만들어 가는 서비스예요. 좋았던 점이나 아쉬웠던 점을 편하게 들려주세요. 짧은 한마디도 저희에겐 큰 힘이 됩니다.'
          ...
          {invitation ? (
            <div className="feedback-dialog__invitation">
              <div className="feedback-dialog__actions">
                <Dialog.Close asChild>
                  <button ref={invitationCloseRef} type="button" className="feedback-dialog__secondary">닫기</button>
                </Dialog.Close>
                <button type="button" className="feedback-dialog__primary" onClick={() => {
                  setInvitation(false);
            ...
            <form className="feedback-dialog__form" onSubmit={handleSubmit} noValidate>
              ...
              <textarea
                ...
                rows={6}
                value={feedback}
                placeholder="좋았던 점, 불편했던 점, 필요한 기능을 짧게 남겨주세요."
