...
- `Modal / CharDetailModal (삭제 확인)` ✅ — NVM-47에서 캐릭터 통째로 삭제 기능 추가(`S1Dashboard.tsx`). 헤더 우측에 삭제 버튼 추가, 클릭 시 "정말 삭제할까요?" + 취소/삭제(빨간 버튼) 확인 단계로 전환
...
- NVM-154에서 후보 리스트에 보유정보/시간정보 충돌 예시 2건 추가(`S1Dashboard.tsx`가 아니라 `mockEpisodeData.ts` 변경에 대응) — "강민준 · 보유 아이템"(권총 분실 등 다중 아이템), "수아 · 시간 경과"(3년 점프, 나이 재계산 필요)
