# Pencil 활성 화면 동기화 트래킹

`design/catchhole.pen`은 CatchHole Front의 실제 브라우저 화면을 기준으로 유지하는 디자인 기준 파일이다. 활성 화면은 `Active / ...` 루트 프레임으로 구분하고, 현재 Front에 연결되지 않은 Post-MVP 시안은 별도 Presentation 영역에 그대로 보존한다.

## 기준과 범위

### 2026-09-08 단일 회차 업로드 권장 안내 — Pencil 반영 대기

`/episode-upload`의 업로드 방식 카드에서 `단일 회차 업로드` 카드 왼쪽 상단에 `추천` 배지를 추가하고, 카드 안에 `가장 정확한 설정 분석을 위해 한 회차씩 업로드하는 것을 권장해요.` 안내를 반영할 예정이다. 설정 분석 정확도를 기준으로 단일 회차 방식을 권장하며, 다회차 파일 분리와 나머지 업로드 방식의 선택·진행 계약은 유지한다.

- 동기화 대상: 업로드 방식 선택과 방식 카드가 표시되는 각 입력 단계의 데스크톱·모바일 원본 프레임. 기존 문서의 대표 선택 화면은 `jVn52`와 `Wmbk0`이며, 실제 대상 노드는 MCP 연결 복구 후 확인한다.
- 함께 갱신할 자료: 해당 원본 프레임 PNG와 원본을 참조하는 업로드 Workflow 복제본. 복제본은 원본 수정 후 다시 생성한다.
- 현재 상태: **Pencil 반영 대기**. 이번 작업에 앞서 재확인한 `get_app_state({})`와 `read_skill({})`가 모두 `-32603` 및 열린 디자인 파일이 필요하다는 오류를 반환했다. 파일 컨텍스트와 실행 문서를 확보하지 못해 `.pen` 노드 조회·수정·렌더 검증을 수행하지 않았다.
- 구현 기준은 [`업로드 데이터 요구사항`](../docs/data-requirements/upload.md#1-업로드-방식-선택)과 [`회차 업로드 플로우`](../docs/screen-flow.md#5-회차-업로드-플로우)에 기록한다. 아래의 과거 Pencil 동기화·검증 완료 기록은 이번 변경의 반영 완료를 의미하지 않는다.

### 2026-09-06 모바일 조작 방식 보완

`mobileUx20260906` 보드에 320px 브라우저 캡처를 동기화했다. 모바일 대시보드 탭·검토 필터는 선택 메뉴로, 업로드 단계는 번호/라벨의 세로 구성으로 바뀐다. 세계관 직접 입력은 한 열 폼과 고정 저장 영역, 캐릭터 상세는 실제 header 높이에 맞춘 본문 스크롤을 사용한다. 데모 헤더는 전체 브랜드와 한 줄 나가기 버튼을 유지한다. 사용자 교정에 따라 데모 진입·새 화면·재체험은 맨 위에서 읽기 시작하고, 안내 대상과 화면 회전에 의한 자동 스크롤은 제거했다. 캡처의 데모 상태도 맨 위로 갱신했다. 실제 기기/Safari 검증을 완료했다는 의미는 아니다.

- 기준 브랜치: `feat/nvm-303-legal-documents`
- 기준 화면 크기: 데스크톱 `1440×900`, 모바일 `390×844`
- 동기화 대상: `/landing`, `/login`, `/signup`, `/demo`, `/terms`, `/privacy`, `/works`, `/dashboard`, `/editor`, `/episode-upload`, `/setting-review`
- 함께 관리하는 상태: 접근 가능한 모달·서랍·토스트, 로딩·빈 화면·오류, 제출·저장·분석·재시도, 토큰 중단·재개
- 제외 대상: `/chat`, `/loading`, `/report`, `/episode-validation-report` 및 현재 `/works`로 리다이렉트되는 미래 화면
- NVM-303의 `/terms`, `/privacy`, 회원가입 법률 문서 상태는 Front·Backend API와 Pencil을 함께 변경하고, 실제 문서 원문·현재 버전은 Backend `legal_documents`를 런타임 기준으로 사용한다.

## 보존된 Post-MVP 시안

아래 프레임은 활성 화면 동기화 대상이 아니며 내용과 구조를 변경하지 않는다.

| 프레임 | ID | 상태 |
| --- | --- | --- |
| 오류 리포트 | `i8TgH` | ✅ 렌더·레이아웃 확인 |
| AI 챗봇 | `N0gSOX` | ✅ 렌더·레이아웃 확인 |
| 인물 관계도·해시태그 그래프 | `XQkp1` | ✅ 렌더·레이아웃 확인 |

## 의미 토큰과 공통 컴포넌트

`theme-v2.css`의 `--ch-*` 의미를 Pen 변수로 대응했다. 핵심 변수는 `ch-canvas`, `ch-surface`, `ch-surface-soft`, `ch-border`, `ch-text`, `ch-text-muted`, `ch-primary`, `ch-success`, `ch-warning`, `ch-danger`이며, 상호작용 색·반경·그림자 토큰도 함께 유지한다.

| 공통 요소 | Pencil ID | 상태 |
| --- | --- | --- |
| Primary / Secondary / Ghost 버튼 | `T8pUh`, `j3kQPk`, `mtajb` | ✅ |
| 입력 / 카드 / 상태 배지 | `N9E7hl`, `Kp23S`, `Idg2H` | ✅ |
| 업데이트 예정 토스트 / 공통 모달 | `xRFAE`, `R3eJi2` | ✅ |
| 데스크톱 / 모바일 상단 바 | `V3TEEV`, `c05VS` | ✅ |
| 데스크톱 사이드바 | `l5cdMk`(기본), `QEAkQ`(대시보드) | ✅ |
| 데스크톱 / 모바일 셸 | `apls8`(기본), `ZbKLd`(대시보드), `jbaEV`(모바일) | ✅ |
| 모바일 서랍 오버레이 | `bxT30`(기본), `OYbvG`(대시보드) | ✅ |

공통 시스템의 캔버스 구역은 `X5NAI`이다. Hover, focus, disabled는 전체 화면 복제가 아니라 공통 상태 보드 `UWUJD`와 컴포넌트 variant로 표현한다.

## 활성 캔버스 구역

활성 화면은 기능 단위로 찾을 수 있도록 좌측 기존 캔버스와 같은 2열 구조로 배치한다. 왼쪽 열은 공통 시스템·작품·업로드·캐릭터/설정집 기능을, 오른쪽 열은 진입·인증·대시보드 공통·세계관·에디터·설정 검토를 쌓는다. 화면 내용이나 구조는 바꾸지 않고, 각 섹션 안에서 데스크톱 화면을 가로 4개까지 놓은 뒤 모바일과 의미 있는 상태 프레임을 다음 행에 이어서 관리한다.

| 캔버스 열 | 순서 | 섹션 | Pencil ID |
| --- | ---: | --- | --- |
| 왼쪽 (`x=76540`) | 10 | 활성 화면 동기화 공통 시스템 | `X5NAI` |
| 왼쪽 (`x=76540`) | 14 | `/works` · 작품 관리 | `k53NoP` |
| 왼쪽 (`x=76540`) | 20 | `/episode-upload` · 업로드 | `cwR3W` |
| 왼쪽 (`x=76540`) | 21 | `/episode-upload` · 분석 상태 | `u7dwvT` |
| 왼쪽 (`x=76540`) | 16 | `/dashboard` · 캐릭터 설정 | `LFNii` |
| 왼쪽 (`x=76540`) | 18 | `/dashboard` · 설정집·검색 | `p40m6e` |
| 오른쪽 (`x=84140`) | 11 | `/landing` · 제품 데모 | `j6KjY` |
| 오른쪽 (`x=84140`) | 12 | `/login` · 로그인 상태 | `TRlEr` |
| 오른쪽 (`x=84140`) | 13 | `/signup` · 회원가입·약관·인증 확인 | `iIpPd` |
| 오른쪽 (`x=84140`) | 25 | `/terms`, `/privacy` · 공개 법률 문서 | `LdqG1` |
| 오른쪽 (`x=84140`) | 15 | `/dashboard` · 셸·원고·분석 | `EFXTg` |
| 오른쪽 (`x=84140`) | 17 | `/dashboard` · 세계관 설정 | `h45eTZ` |
| 오른쪽 (`x=84140`) | 19 | `/editor` · 원문 보기 | `QIrnO` |
| 오른쪽 (`x=84140`) | 22 | `/setting-review` · 캐릭터 후보 | `QWUxR` |
| 오른쪽 (`x=84140`) | 23 | `/setting-review` · 세계관 후보 | `w4ntpW` |
| 오른쪽 (`x=84140`) | 24 | `/demo` · 인터랙티브 코치마크 | `A3226` |

## 활성 화면 현황

활성 루트 프레임은 총 214개이며 모두 `Active / ...` 이름을 사용한다.

| 화면군 | 루트 수 | 포함 상태 | 대표 ID |
| --- | ---: | --- | --- |
| 랜딩·인증·법률 문서·작품 관리 | 54 | 랜딩 데모 1–8, 로그인, 휴대폰 인증 회원가입, 현재 법률 문서 로딩/성공/오류, 약관/개인정보 모달·공개 페이지, 인증 확인/오류, 작품 목록/모달 | `y2tKZo`, `u2KiIK`, `R9BLkK`, `virQv`, `NkhK3`, `GfeiI`, `U7bWrQ`, `px4Sl`, `gnkyK`, `M5zI8e`, `lMEt9`, `MZhgx` |
| 대시보드 | 56 | 원고·분석, 모바일 서랍, 캐릭터, 세계관, 설정집, 설정 검색, 업데이트 예정 토스트 | `RnXjQ`, `DY6xk`, `qwIxE`, `RSX1d`, `K6GRx`, `ReWhO` |
| 원문·회차 업로드 | 46 | 원문 조회/오류, 업로드 방식 3종, 회차 감지/수정/확정, 저장 오류, 분석·토큰 상태 | `q4eLeC`, `jVn52`, `ovaQg`, `rPfF6`, `rPbgJ` |
| 설정 검토 | 40 | 캐릭터 연결/수정/비교/확정/무시, 세계관 일괄·개별 수정/충돌/재비교/토큰 중단/완료 | `GEwV9`, `CLEWm`, `la0t2`, `OqOrE`, `S3QmB` |
| 인터랙티브 데모 | 18 | 10문단 원고 확인, 실제형 fixture 분석, 후보 확정·수정안 적용·제외, 5명 캐릭터 DB 카드·상세·원문 근거·변화 이력, 세계관 DB 설정·원문 근거, 완료, 모바일 대응 상태 | `kPwVB`, `Qw9ms`, `PRgXY`, `wZWek`, `Ml4QD`, `oNTuG`, `V73qOf`, `Asueh`, `XwlRJ`, `jNNUD`, `N3XmT`, `iRL65`, `oHdl5`, `W5Fbyl`, `qkAJB`, `uMDMJ`, `C1fqUz`, `ToEIU` |

### 진입·인증·작품 관리

- 랜딩은 자동 재생되는 제품 데모 8단계를 각각 고정 프레임으로 두고 모바일 구조를 별도 프레임으로 유지한다.
- 랜딩의 `omdLandingManuscriptNotice`는 제품 데모 아래, 핵심 기능 3항목 요약 위에 항상 표시하는 원고 학습 미사용 안내의 편집 가능한 참조 프레임이다. 제목 `작가님의 원고는 AI 학습에 사용하지 않습니다.`, 본문 `원고와 분석 결과 모두에 적용됩니다.`, `/privacy` 링크를 유지한다. 캔버스 배경 위 데스크톱 제목은 32px·위아래 여백은 56px이며 모바일은 제목 26px·여백 40px로 줄인다. 직접 연결 주소는 `/landing#manuscript-protection`이다.
- 로그인은 기본·검증 오류·제출 중, 회원가입은 기본·인증번호 입력·재전송 대기·인증 완료·오류를 데스크톱과 모바일로 구분한다.
- 회원가입은 Backend의 현재 게시 이용약관·개인정보처리방침을 조회한 상태와 만 14세 이상 필수 확인을 표시한다. 현재 문서 로드됨·로딩·실패 대표 프레임은 `virQv`, `NkhK3`, `GfeiI`다.
- 법적 고지 모달은 이용약관과 개인정보처리방침 탭을 각각 보존하고 `2026-08-24` 게시본을 표시한다. 공개 전체 페이지는 데스크톱 이용약관 `px4Sl`, 개인정보처리방침 `gnkyK`, 모바일 개인정보처리방침 `M5zI8e`를 기준으로 한다.
- GA4·Meta 고지는 개인정보처리방침 본문 안에만 두며 별도 쿠키 배너나 회원가입 선택 체크박스 프레임을 만들지 않는다.
- 작품 관리는 목록·로딩·빈 화면·조회 실패와 생성·수정·영구 삭제 모달을 데스크톱/모바일로 구분한다.

### 대시보드

- 공통 상단 영역, 사이드바, 남은 사용량과 모바일 서랍을 공통 셸에서 재사용한다.
- 원고 목록은 목록·빈 화면·오류, 분석 목록은 완료·진행·대기·부분 실패·실패 상태를 포함한다.
- 캐릭터는 목록·빈 화면·상세·수정·삭제·변화 이력·원문 근거를 포함한다.
- 세계관은 분류 선택, 목록/상세, 생성/수정, 충돌, 미저장 변경을 포함한다.
- 설정집은 목록·원문·업로드·수정·삭제·오류를 포함한다.
- 설정 검색은 기본·결과·상세 모달을 포함하며, 접근 가능한 미래 기능은 `업데이트 예정` 토스트로만 표현한다.

### 원문·회차 업로드

- `/editor`는 원문 조회·로딩·조회 오류·변환 오류·접근 오류를 포함한다.
- `/episode-upload`는 업로드 방식 선택, 단일 회차, 다회차 단일 파일, 다회차 여러 파일, 회차 감지·수정·확정을 포함한다.
- 제출·저장·분석 시작 오류는 작성 데이터 유지와 다음 행동이 다르므로 별도 프레임으로 둔다.
- 분석은 진행·대기·성공·부분 실패·전체 실패·재시도, 토큰 중단·재개를 별도 프레임으로 둔다.

### 설정 검토

- 캐릭터 후보는 검토, 연결 필요, 연결 모달, 수정, 비교, 잘못된 값, 확정, 무시, 완료를 포함한다.
- 세계관 후보는 검토, 일괄 수정, 개별 수정, 충돌, 재비교 필요/중/성공/실패, 토큰 중단, 완료를 포함한다.
- 모바일은 후보 목록과 상세를 별도 프레임으로 분리하여 실제 전환 구조를 표현한다.

### 인터랙티브 데모

- `/demo`는 로그인 없이 진입하며, 예시 원고와 사전 계산된 fixture만 사용한다.
- 데스크톱은 10문단 원고 확인 → 분석 → 후보 검토 → 5명 캐릭터 DB 카드·상세·현재 근거·회차별 변화 이력 확인 → 세계관 DB 설정·근거 확인 → 완료 상태를 각각 고정 프레임으로 둔다.
- 분석은 실제 회차 업로드 화면과 같은 처리 단계·상태명·`설정 후보 검토` CTA를 사용한다.
- 후보 검토에서 실제 화면의 `1개 설정 모두 확정`, `수정`, `수정안 적용`, `모두 확정`, `제외`를 직접 수행하며 존재하지 않는 보조 기능은 추가하지 않는다.
- 코치마크는 화면 전체를 어둡게 가리지 않고 한 번에 하나의 다음 행동만 안내한다. 대상 바깥 윤곽선은 글자를 덮지 않으며, 코치마크 아래의 실제 화면 조작도 막지 않는다. 현재 안내와 무관한 캐릭터·근거·이력을 열어도 문구와 단계는 바뀌지 않는다.
- 결과 탐색은 별도 데모 시안을 복제하지 않고 배포 화면의 `CharacterDatabase`, `CharacterTimelineModal`, `WorldSettingDatabase`에 fixture를 주입한다. 에단 렌 상세에서는 현재 직업 근거를 확인하고, 변화 이력을 연 뒤 현재 설정의 `직업`을 선택해 1화·6화 실제 타임라인과 6화 원문 근거를 확인한다. 세계관 설정 DB에서는 실제 모바일 목록/상세 전환과 설정 행 근거 펼치기를 그대로 사용한다.
- 완료 화면의 `내 작품으로 시작하기`는 `/signup`으로 현재 히스토리를 대체하고, 회원가입 모달 X는 데모를 다시 마운트하지 않고 `/landing`으로 이동한다.
- 모바일은 원고·코치마크, 세로로 쌓은 5명 DB 카드, 캐릭터 상세·원문 근거·변화 이력, 세계관 원문 근거, 완료·가입/재체험을 별도 프레임으로 유지한다.
- 전체 전환과 fixture·무 API·초기화 계약은 `Workflow Board / WF-06 인터랙티브 데모`(`BE15b`)에서 관리한다.

## 상태 분리 기준

- 레이아웃, 정보 계층, 사용자 액션, 상태 의미, 모달·서랍 또는 모바일 구조가 달라지면 별도 루트 프레임을 사용한다.
- 샘플 데이터, 서버 내부 원인, 페이지 번호만 다르고 UI와 다음 행동이 같으면 대표 프레임으로 통합한다.
- 브라우저 렌더러의 사소한 픽셀 차이는 복제하지 않고 의미 토큰, 문구, 액션, 상태 전이를 우선한다.
- 실제 사용자 데이터 대신 기존 Playwright API mock의 대표 데이터를 사용한다.

## 이미지와 Workflow Board

- 모든 활성 루트 프레임 PNG는 `docs/screens/<Pencil-node-id>.png`에 내보낸다.
- 기존 활성 화면군과 별도로 `/demo` 활성 루트 18개의 PNG를 모두 내보낸다.
- `/demo` 흐름은 `docs/workflows/WF-06.png`와 `docs/screen-flow.md`를 함께 갱신한다.

## 검증

- 각 화면군 생성 직후 `TakeScreenshot`으로 렌더를 확인했다.
- 각 루트에서 `SnapshotLayout(problemsOnly: true)`와 동일한 문제 방문 검사를 수행했다.
- 감지된 텍스트 잘림, 상태 배지 잘림, 모바일 비교/편집 화면 넘침은 즉시 보정하고 다시 렌더했다.
- 실제 Front 캡처와 대표 Pen PNG를 비교해 워드마크 크기, 대시보드 사이드바 활성 항목, 작품 장르 표기도 최종 보정했다.
- 최종 코드 검증은 `npm run lint`, `npm run build`, `npm run test:e2e` 순서로 실행한다.

## 실제 Front UI·접근성 문제

- 현재 동기화 과정에서 제품 문제로 확정해 기록할 재현 항목은 없다.
- 자동 캡처 과정에서 생성된 빈 검정 PNG는 페이지 UI 문제가 아니라 캡처 실패 자료이므로 이 목록에 포함하지 않는다.
