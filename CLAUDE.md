...
| `/setting-review` | `SSettingReview` | 업로드 묶음에서 추출된 캐릭터·세계관 후보를 `candidateType=character\|world`로 나눠 검토한다. `?workId=<id>&batchId=<id>&jobType=<EPISODE_VALIDATION\|SETTING_EXTRACTION>&group=<key>&candidate=<id>`로 검토 문맥·선택 그룹·후보를 딥링크하며, 두 탭의 후보 DTO와 mutation은 분리한다. 캐릭터 후보는 정규화한 이름별 그룹으로 표시하고 `미상`은 마지막에 두며 그룹 안 모든 row를 세로로 이어서 검토한다. row별 설정 내용 수정·제외·단건 캐릭터 연결, 그룹 캐릭터 일괄 연결, 그룹 전체 확정을 제공하고 일반 수정에서는 캐릭터 이름을 바꾸지 않는다. 기존 캐릭터와 비교된 후보는 기존값·제안값·AI 판단을 함께 표시하고 `APPLY_PROPOSAL` 또는 `HISTORY_ONLY`를 선택한다. 처리 중 비교는 전체 확정을 잠그고 실패·재비교 필요 및 연결된 레거시 `NOT_REQUIRED` 상태는 retry한다. 신규 등록 예정 `UNRESOLVED` 그룹은 한 캐릭터로 함께 생성하며, 동일 이름 기존 캐릭터가 발견되면 비교 후 재확정한다. 상단 진행률은 두 후보 집계를 합산하고 뒤로가기는 해당 작품의 `nav=analyses`로 돌아간다. |
...
- **첫 분석 안내 예시**: 인증된 업로드 화면의 `AnalysisModeGuideDialog`는 실제 업로드·검토·프로필 컴포넌트에 가상의 데이터를 넣은 읽기 전용 예시다. PNG 캡처 대신 컴포넌트를 공유하고 예시의 조작은 inert로 차단한다. 실제 분석·설정 API는 호출하지 않으며 안내 대상/계정당 1회 기록만 실제 생성 SDK를 사용한다. 서버 데이터 조회 실패를 예시로 대신하지 않는다. [노출·URL·컴포넌트 공유 계약](docs/analysis-mode-guide.md).
