...
- **첫 분석 안내 예시**: 인증된 업로드 화면의 `AnalysisModeGuideDialog`는 예시임을 표시한 메모리 실습이다. 실제 분석·설정 API는 호출하지 않으며 안내 대상/계정당 1회 기록만 실제 생성 SDK를 사용한다. 서버 데이터 조회 실패를 예시로 대신하지 않는다. [노출·URL·실습 계약](docs/analysis-mode-guide.md).
