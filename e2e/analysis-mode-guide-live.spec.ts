...
test('첫 분석 안내는 실제 계정에 한 번 기록하고 예시 화면 탐색은 실제 작품에 저장하지 않는다', async ({ page, request }) => {
  ...
  try {
    ...
    const dialog = page.getByRole('dialog', { name: '설정 반영 방식을 비교해 보세요' });
    ...
    await expect.poll(() => dialog.getByRole('img').evaluate((node: HTMLImageElement) => node.complete && node.naturalWidth > 0)).toBe(true);
    await page.screenshot({ path: 'docs/screens/gh194/analysis-guide-desktop.png', animations: 'disabled' });
    await page.keyboard.press('ArrowRight');
    await dialog.getByRole('tab', { name: '모든 설정 직접 검토' }).click();
    await expect(dialog.getByRole('heading', { name: '3개 설정을 모두 살펴보고 확정해요' })).toBeVisible();
    await expect(dialog.locator('input, select, textarea')).toHaveCount(0);
    await dialog.getByRole('button', { name: '다음', exact: true }).click();
    await expect(dialog.getByRole('heading', { name: '모두 직접 확정한 뒤 3개가 저장돼요' })).toBeVisible();
    await dialog.getByRole('tab', { name: 'AI 판단으로 설정 자동 반영' }).click();
